...
Usage::
    ...
    from thegent.orchestration.dag_prioritization import DagPrioritizer, DagTask
...
class DagPrioritizer:
    ...
    def _kahn_sort(self) -> list[str]:
        ...
        if len(order) != len(self._tasks):
            raise DagCycleError(
                "Cycle detected in task graph; topological sort is not possible."
            )
    ...
    def get_priority_score(self, task_id: str) -> float:
        ...
        for tid in reversed(topo_order):
            if successors[tid]:
                lf[tid] = min(
                    lf[succ] - self._tasks[succ].estimated_duration_s
