...
class _InMemoryStore:
    ...
    def count_with_prefix_sync(self, prefix: str) -> int:
        ...
        now = time.monotonic()
        return sum(1 for k, exp in self._active.items() if k.startswith(prefix) and exp >= now)
...
class RedisConcurrencyController:
    ...
    def __init__(
        ...
        self._config = redis_config or RedisConfig.from_settings()
        if max_concurrent is None:
            settings = ThegentSettings()
            self._max_concurrent = settings.redis_concurrency_limit
        else:
    ...
    def _try_connect_redis(self) -> None:
        ...
        if aioredis is None:
            _log.debug("redis_concurrency: redis package not installed, using in-memory fallback")
    ...
    async def release(self, run_id: str) -> None:
        ...
        if self._redis_available and self._redis is not None:
            try:
                ...
                _log.debug("redis_concurrency: released run_id=%s key=%s", run_id, key)
                return
            except Exception as exc:
                _log.warning("redis_concurrency: release failed, switching to fallback: %s", exc)
                self._switch_to_fallback()

        if self._fallback is not None:
            await self._fallback.delete(key)
            _log.debug("redis_concurrency: fallback released run_id=%s key=%s", run_id, key)
    ...
    async def _try_acquire(self, key: str) -> bool:
        ...
        if self._redis_available and self._redis is not None:
            try:
                ...
                _log.warning("redis_concurrency: acquire failed, switching to fallback: %s", exc)
                self._switch_to_fallback()

        if self._fallback is not None:
            return await self._fallback.setnx_bounded(key, self._slot_ttl_s, self._max_concurrent)
        return False
    ...
    async def _try_acquire_redis(self, key: str) -> bool:
        ...
        result = await self._redis.set(key, "1", nx=True, ex=int(self._slot_ttl_s))
        if not result:
            return False
    ...
    def _switch_to_fallback(self) -> None:
        ...
        if self._fallback is None:
            self._fallback = _InMemoryStore()
        _log.warning("redis_concurrency: switched to in-memory fallback (Redis unavailable)")
