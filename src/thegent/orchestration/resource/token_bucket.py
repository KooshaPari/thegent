...
class TokenBucket:
    ...
    def __init__(self, config: TokenBucketConfig) -> None:
        self._config = config
        self._tokens: float = config.capacity if config.initial_tokens is None else config.initial_tokens
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    cond_wait: float | None = (
                        min(wait_for_refill, remaining) if wait_for_refill is not None else remaining
                    )
                else:
...
class RateLimitedSwarmRunner:
    ...
    def configure_from_env(self) -> RateLimitedSwarmRunner:
        ...
        rate = float(settings.rate_tokens_per_sec)
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        effective_timeout: float | None = self._default_timeout_s if timeout_s is _UNSET else timeout_s
        ...
        if not acquired:
            msg = f"Rate limit exceeded: could not acquire token within {effective_timeout}s"
