...
def _get_memory_mb_macos_vm_stat() -> float:
    ...
    Page size is read from ``sysctl hw.pagesize``; falls back to 4096 bytes.

    Returns 512.0 as a conservative sentinel on any parse or subprocess failure.
    """
    try:
        ...
        for line in out.stdout.splitlines():
            ...
            if "Pages free:" in line:
                ...
                raw = line.split(":")[1].strip().rstrip(".")
                if raw.isdigit():
                    pages_inactive = int(raw)
            elif "Pages speculative:" in line:
                raw = line.split(":")[1].strip().rstrip(".")
                if raw.isdigit():
                    pages_speculative = int(raw)
            elif "Pages purgeable:" in line:
                raw = line.split(":")[1].strip().rstrip(".")
                if raw.isdigit():
                    pages_purgeable = int(raw)
...
def _get_memory_mb() -> tuple[float, float]:
    ...
    - macOS: parse ``vm_stat`` (pages_free + pages_inactive) * hw.pagesize
    - Linux: ``/proc/meminfo`` MemAvailable field
    - Other: conservative 512 MB sentinel
    """
    rss_mb = 0.0
    available_mb = 512.0
    try:
...
class HysteresisController:
    ...
    def __init__(
        ...
        self.upper_threshold = (
            upper_threshold if upper_threshold is not None else settings.hysteresis_upper
        )
        self.lower_threshold = (
            lower_threshold if lower_threshold is not None else settings.hysteresis_lower
        )
        self.dwell_time_s = (
            dwell_time_s if dwell_time_s is not None else settings.hysteresis_dwell
        )
...
class DeadlineMonitor:
    ...
    def _emit_overdue(dl: SoftDeadline, elapsed: float) -> None:
        ...
        msg = (
            f"soft_deadline exceeded: run_id={dl.run_id} "
            f"elapsed={elapsed:.1f}s deadline={dl.deadline_ts:.1f}s"
        )
        try:
