...
class HarnessCard:
    ...
    memory_base: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    memory_per_session: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    
    # File descriptors: statistical distribution
    fd_base: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    fd_per_session: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    
    # CPU: statistical distribution
    cpu_base: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    cpu_per_session: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    
    # Child processes: statistical distribution
    child_process_base: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    child_process_per_session: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    
    # Threads: statistical distribution
    thread_base: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    thread_per_session: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    
    # Network: statistical distribution
    network_bytes_per_request: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    network_connections_per_session: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    
    # Sockets: statistical distribution
    socket_base: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    socket_per_session: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
    
    # Latency: statistical distribution
    latency: ResourceDistribution = field(default_factory=lambda: ResourceDistribution())
...
def sample_extended_resources() -> ExtendedResourceSnapshot:
    ...
    try:
        ...
        try:
            ...
            snapshot.child_process_memory_mb = sum(
                c.memory_info().rss / (1024 * 1024) for c in children
            )
            snapshot.child_process_fd_count = sum(
                c.num_fds() if hasattr(c, "num_fds") else 0 for c in children
            )
            
            # Count zombie processes
            snapshot.zombie_process_count = sum(
                1 for c in children if c.status() == psutil.STATUS_ZOMBIE
            )
...
def detect_leaks(
    history: deque[ExtendedResourceSnapshot],
    current: ExtendedResourceSnapshot,
