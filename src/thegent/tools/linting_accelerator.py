...
class LintingAccelerator:
    ...
    def run_oxlint(
        ...
        if not self.is_oxlint_available():
            raise FileNotFoundError(
                "oxlint not found on PATH. "
                "Install via: npm install -g oxlint  or  cargo install oxlint"
            )
    ...
    def run_eslint(
        ...
        if not self.is_eslint_available():
            raise FileNotFoundError(
                "eslint not found on PATH. "
                "Install via: npm install -g eslint  or  bun add -d eslint"
            )
    ...
    def run_ruff(self, paths: list[Path]) -> list[LintResult]:
        ...
        if not self.is_ruff_available():
            raise FileNotFoundError(
                ...
                "Install via: pip install ruff  or  uv tool install ruff"
            )
