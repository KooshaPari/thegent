...
def get_agent_id() -> str:
    """Return the current agent ID from env or default."""
    return os.environ.get("THGENT_AGENT_ID", "default-agent")
...
def commit(
    ...
    if not ref.startswith("refs/"):
        try:
            full_ref = subprocess.check_output(
                ["git", "symbolic-ref", "-q", ref], cwd=project_root, text=True
            ).strip()
    ...
    try:
        old_hash = subprocess.check_output(
            ["git", "rev-parse", full_ref], cwd=project_root, text=True
        ).strip()
    except subprocess.CalledProcessError:
    ...
    if manager.update_ref_cas(full_ref, new_hash, old_hash):
        ...
        console.print(f"[bold red]Update collision detected![/bold red]")
        console.print("[yellow]The reference moved while we were committing. You may need to merge/rebase and retry.[/yellow]")
        raise typer.Exit(1)
...
def log(
    ...
    try:
        output = subprocess.check_output(
            ["git", "log", f"-n{limit}", "--oneline", "--graph", "--decorate"],
            cwd=project_root,
            text=True
        )
