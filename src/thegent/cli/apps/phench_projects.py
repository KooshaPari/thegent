...
def _ensure_selected_ref(
    ...
    *,
    ...
    if no_interactive:
        raise typer.BadParameter("--ref/--branch is required when --no-interactive is set for single repo selection")
    ...
    for spec in repo_ref_specs:
        if "@" not in spec:
            raise typer.BadParameter(
                "repo-ref entries must use the format <repo-id>@<ref> (for example repo-a@feature-branch)"
            )
...
def _build_projects_plan_kwargs(
    *,
    ...
    lock_repo_ids_all = {repo.repo_id for repo in lock.repos}

    def _match_repo_ids(patterns: list[str] | None, repo_ids: list[str]) -> list[str]:
        if not patterns:
            return list(repo_ids)
        selected: list[str] = []
        seen: set[str] = set()
        for pattern in patterns:
            for candidate_repo_id in repo_ids:
                if fnmatch(candidate_repo_id, pattern) and candidate_repo_id not in seen:
                    selected.append(candidate_repo_id)
                    seen.add(candidate_repo_id)
        return selected

    effective_repo_ids = sorted(lock_repo_ids_all)
    if include_repos:
        effective_repo_ids = _match_repo_ids(include_repos, effective_repo_ids)
    if exclude_repos:
        effective_repo_ids = [
            repo_id for repo_id in effective_repo_ids if not any(fnmatch(repo_id, pattern) for pattern in exclude_repos)
        ]
    if not effective_repo_ids:
        raise typer.BadParameter("repo filters resolved to no repositories")

    lock_repo_ids = set(effective_repo_ids)
    filtered_lock_repos = [repo for repo in lock.repos if repo.repo_id in lock_repo_ids]
    ...
    if module is not None:
        ...
        module_repo_ids = [repo_id for repo_id in module_repo_ids if repo_id in lock_repo_ids]
        if not module_repo_ids:
            raise typer.BadParameter(f"module {module} has no repos after filters")
        lock_repo_ids = set(module_repo_ids)
        filtered_lock_repos = [repo for repo in filtered_lock_repos if repo.repo_id in lock_repo_ids]
    ...
    *,
    ...
    def projects_run_cmd(
        ...
        no_interactive: bool = typer.Option(
            False, "--no-interactive", help="Fail if command selection would be interactive."
        ),
    ...
    def projects_modules_cmd(
        ...
        target: str | None = typer.Option(
            None, "--target", help="Optional target to validate module repos against lock."
        ),
        family: str | None = typer.Option(None, "--family", help="Optional target family namespace."),
    ) -> None:
        if module is None:
