...
def serialize_health_trend_csv(result: dict[str, Any]) -> str:
    ...
    writer.writerow(
        [
            ...
            result.get("scope_key_json", json.dumps(result.get("scope_key", {}).decode(), sort_keys=True)),
            result.get("delta_summary_json", json.dumps(result.get("delta_summary", {}).decode(), sort_keys=True)),
            ...
            json.dumps(_safe_dict(result.get("compat").decode()).get("aliases", {}), sort_keys=True),
            compat_aliases_count,
        ]
    )
