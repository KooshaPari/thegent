...
def thegent_escalate_list_impl(
    *,
    past_sla_only: bool,
    limit: int,
...
def thegent_escalate_add_impl(
    *,
    ...
    elapsed_ms = int((time.perf_counter() - start_time) * 1000)
    return ToolResult(
        content=json.dumps({"success": True, "run_id": run_id}).decode().decode(),
        structured_content={"success": True, "run_id": run_id},
        meta={"execution_time_ms": elapsed_ms},
    )


def thegent_escalate_approve_impl(
    *,
    run_id: str,
    ...
    elapsed_ms = int((time.perf_counter() - start_time) * 1000)
    return ToolResult(
        content=json.dumps({"success": ok, "run_id": run_id}).decode().decode(),
        structured_content={"success": ok, "run_id": run_id},
        meta={"execution_time_ms": elapsed_ms},
    )


def thegent_escalate_resolve_impl(
    *,
    run_id: str,
    resolution: str,
...
def thegent_govern_list_pending_impl(
    *,
