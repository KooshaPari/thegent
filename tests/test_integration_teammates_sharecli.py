"""Integration tests for TeammateManager with ShareCLI coordination (WP-16003).

Tests the full integration between:
- TeammateManager delegation
- ShareCLI Bridge task coordination
- Intent broadcasting
...
class TestTeammateManagerShareCLIIntegration:
    """Integration tests for TeammateManager with ShareCLI Bridge."""

    @pytest.fixture
    def harness_root(self, tmp_path):
        """Create a temporary harness root for ShareCLI."""
    ...
    @pytest.fixture
    def sharecli_bridge(self, harness_root, monkeypatch):
        """Create a ShareCLIBridge with mocked harness root."""
        monkeypatch.setenv("HARNESS_ROOT", str(harness_root))
        return ShareCLIBridge()

    def test_delegation_creates_sharecli_task(self, teammate_manager, sharecli_bridge, harness_root):
        """WP-16003: Delegation should create a task in ShareCLI when available."""
        # Verify ShareCLI is available
        assert sharecli_bridge.is_available() is True
        ...
        # Verify task was created in ShareCLI
        task_dir = harness_root / "var" / "tasks"
        task_file = task_dir / req.id
        assert task_file.exists(), "Task file should be created in ShareCLI"
    ...
    def test_delegation_broadcasts_intent(self, teammate_manager, sharecli_bridge, harness_root):
        """WP-16003: Delegation should broadcast intent to ShareCLI mesh."""
        # Create a delegation
        req = teammate_manager.delegate(
    ...
    def test_get_session_state_finds_delegations(self, teammate_manager, sharecli_bridge, harness_root):
        ...
        # Get session state
        state = sharecli_bridge.get_session_state(session_id)
    ...
    def test_delegation_works_without_sharecli(self, teammate_manager, tmp_path, monkeypatch):
        """WP-16003: Delegation should work gracefully when ShareCLI is not available."""
        ...
        req = teammate_manager.delegate(
            ...
            prompt="This should work without ShareCLI.",
        )
    ...
    def test_multiple_delegations_create_multiple_tasks(self, teammate_manager, sharecli_bridge, harness_root):
    ...
    def test_delegation_task_includes_dependencies(self, teammate_manager, sharecli_bridge, harness_root):
    ...
    def test_session_state_empty_when_no_delegations(self, sharecli_bridge):
        """WP-16003: get_session_state should return empty state for session with no delegations."""
        state = sharecli_bridge.get_session_state("NONEXISTENT-SESSION")

        assert state == {"claims": [], "intents": [], "tasks": []}

    def test_intent_file_format(self, teammate_manager, sharecli_bridge, harness_root):
    ...
    def test_task_file_format(self, teammate_manager, sharecli_bridge, harness_root):
    ...
    def test_delegation_status_update_preserves_sharecli_task(self, teammate_manager, sharecli_bridge, harness_root):
        """WP-16003: Updating delegation status should not affect ShareCLI task."""
