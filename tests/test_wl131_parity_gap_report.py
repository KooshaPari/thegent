...
def test_parity_gap_report_exists() -> None:
    ...
    assert REPORT_PATH.exists(), (
        f"Expected parity gap report at {REPORT_PATH}"
    )
...
def test_parity_file_1_exists() -> None:
    ...
    assert PARITY_FILE_1.exists(), (
        f"Expected parity test file at {PARITY_FILE_1}"
    )


def test_parity_file_2_exists() -> None:
    """The Wave-2 Rust/Python parity test file must exist."""
    assert PARITY_FILE_2.exists(), (
        f"Expected parity test file at {PARITY_FILE_2}"
    )
