"""Unit tests for heliosShield Bridge (WP-16003, WP-16004).

Tests for:
- heliosShieldBridge: Task coordination and intent broadcasting
...
class TestheliosShieldBridge:
    """Test heliosShieldBridge functionality (WP-16003)."""
    ...
    def bridge(self, harness_root, monkeypatch):
        """Create a heliosShieldBridge instance with mocked harness root."""
        monkeypatch.setenv("THGENT_HARNESS_ROOT", str(harness_root))
    ...
    def test_is_available_false_when_not_initialized(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("THGENT_HARNESS_ROOT", str(tmp_path / "nonexistent"))
    ...
    def test_is_available_uses_default_path(self, tmp_path, monkeypatch):
        """Test is_available uses default ~/.agent-harness when THGENT_HARNESS_ROOT not set."""
        monkeypatch.delenv("THGENT_HARNESS_ROOT", raising=False)
    ...
    def test_create_shared_task_fails_when_not_available(self, tmp_path, monkeypatch):
        """Test create_shared_task returns False when heliosShield not available."""
        monkeypatch.setenv("THGENT_HARNESS_ROOT", str(tmp_path / "nonexistent"))
        bridge = heliosShieldBridge()
    ...
    def test_broadcast_intent_fails_when_not_available(self, tmp_path, monkeypatch):
        """Test broadcast_intent returns False when heliosShield not available."""
        monkeypatch.setenv("THGENT_HARNESS_ROOT", str(tmp_path / "nonexistent"))
        bridge = heliosShieldBridge()
    ...
    def test_get_session_state_empty_when_not_available(self, tmp_path, monkeypatch):
        """Test get_session_state returns empty state when heliosShield not available."""
