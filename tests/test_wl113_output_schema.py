...
def schema_file(tmp_path: Path) -> Path:
    ...
    p.write_text(json.dumps(SIMPLE_SCHEMA), encoding="utf-8")
    return p
...
def string_schema_file(tmp_path: Path) -> Path:
    ...
    p.write_text(json.dumps(STRING_SCHEMA), encoding="utf-8")
    return p
...
def array_schema_file(tmp_path: Path) -> Path:
    ...
    p.write_text(json.dumps(ARRAY_SCHEMA), encoding="utf-8")
    return p
...
class TestOutputSchemaValidatorSystemPromptInjection:
    ...
    def test_injection_contains_schema_json(self, schema_file: Path) -> None:
        ...
        assert json.dumps(SIMPLE_SCHEMA, indent=2) in injection
