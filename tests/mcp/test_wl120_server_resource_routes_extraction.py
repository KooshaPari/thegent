...
def _register_routes() -> tuple[Any, ...]:
    ...
    sessions = SimpleNamespace(
        ...
        resource_session_meta_impl=lambda **kwargs: json.dumps({"id": kwargs["session_id"]}).decode(),
        resource_session_logs_impl=lambda **kwargs: "ok",
    )
    catalog = SimpleNamespace(
        resource_dag_impl=lambda **kwargs: json.dumps({"tasks": []}).decode(),
        resource_agents_impl=lambda **kwargs: json.dumps([]).decode(),
        resource_models_impl=lambda **kwargs: json.dumps(
            {"provider": kwargs["provider"], "include_contract": kwargs["include_contract"]}
        ),
        resource_models_contract_impl=lambda: json.dumps({"contract": "ok"}).decode(),
    )
    workstream = SimpleNamespace(
        resource_workstream_impl=lambda: "# workstream",
        resource_events_session_complete_impl=lambda: json.dumps({"events": []}).decode(),
        resource_workstream_db_impl=lambda: json.dumps({"db": "ok"}).decode(),
    )
    contracts = SimpleNamespace(
        resource_session_contracts_impl=lambda **kwargs: json.dumps({"summary": {}}).decode(),
        resource_session_contract_health_gate_impl=lambda **kwargs: json.dumps({"gate": "ok"}).decode(),
        resource_session_contract_health_report_impl=lambda **kwargs: json.dumps({"report": "ok"}).decode(),
        resource_session_contract_health_trend_impl=lambda **kwargs: json.dumps({"trend": []}).decode(),
    )
...
def test_resource_routes_registers_expected_handler_set() -> None:
    ...
    mod.register_resource_routes(
        ...
        session_contract_health_report_impl=lambda **kwargs: {},
        session_contract_health_trend_impl=lambda **kwargs: {},
        stable_json=lambda payload: json.dumps(payload).decode(),
    )
