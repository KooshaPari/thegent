...
Covers:
- PowerShell profile path detection (``$PROFILE`` env var, PS7+, PS5 defaults).
- PowerShell environment detection via ``sys.platform`` and ``SHELL`` env var.
- Appending the mise activation hook to an existing and a new profile file.
- Idempotency: hook is not written twice.
- ``install_system_dependencies`` with ``install_powershell=True``.
- Dry-run mode.
