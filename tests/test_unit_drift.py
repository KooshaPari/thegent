...
def test_drift_detector_establishes_policy_baseline(mock_settings):
    ...
    contracts_dir.mkdir()
    (contracts_dir / "policy.json").write_text(json.dumps({"allow": True}).decode().decode(), encoding="utf-8")
...
def test_drift_detector_detects_policy_contract_change(mock_settings):
    ...
    contract_path.write_text(json.dumps({"allow": True}).decode().decode(), encoding="utf-8")

    detector = DriftDetector(mock_settings)
    first_report = detector.detect_drift()
    assert first_report["baseline_established"] is True

    contract_path.write_text(json.dumps({"allow": False}).decode().decode(), encoding="utf-8")
...
def test_drift_detector_detects_policy_contract_add_and_remove(mock_settings):
    ...
    first.write_text(json.dumps({"a": 1}).decode().decode(), encoding="utf-8")
    second.write_text(json.dumps({"b": 2}).decode().decode(), encoding="utf-8")

    detector = DriftDetector(mock_settings)
    detector.detect_drift()

    second.unlink()
    (contracts_dir / "third.json").write_text(json.dumps({"c": 3}).decode().decode(), encoding="utf-8")
...
def test_drift_detector_raises_on_invalid_baseline(mock_settings):
    ...
    baseline.write_text(json.dumps({"generated_at_utc": "2026-02-23T00:00:00+00:00", "contracts": []}).decode().decode(), encoding="utf-8")
