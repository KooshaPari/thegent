...
def test_inject_session_context_returns_string():
    ...
    result = inject_session_context(store, topics=["python"])
...
def test_inject_empty_store():
    store = MagicMock()
    store.get_recent.return_value = []
    result = inject_session_context(store, topics=[])
    assert isinstance(result, str)
    assert len(result) > 0
...
def test_inject_with_hours_and_limit():
    store = MagicMock()
    store.get_recent.return_value = []
    inject_session_context(store, topics=["python"], hours=12, limit=5)
