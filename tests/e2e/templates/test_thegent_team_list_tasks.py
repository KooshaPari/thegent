"""
E2E test for: thegent team list-tasks

Agent Journey: Agent executes thegent team list-tasks command
...
@pytest.mark.e2e
class TestTeamList-tasks:
    """E2E tests for thegent team list-tasks command."""

    def test_team_list_tasks_exits_zero(self) -> None:
        """thegent team list-tasks exits with code 0."""
        result = runner.invoke(app, ['team', 'list-tasks'])
        assert result.exit_code == 0, f"Command failed: {result.stdout} {result.stderr}"

    def test_team_list_tasks_produces_output(self) -> None:
        """thegent team list-tasks produces expected output."""
        result = runner.invoke(app, ['team', 'list-tasks'])
