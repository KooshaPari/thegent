...
def test_fast_deep_lane_doc_exists() -> None:
    ...
    assert LANE_DOCS.exists(), (
        f"FAST_DEEP_LANE.md not found at {LANE_DOCS}. "
        "Create it per WL-134 B90-W3-C2 spec."
    )
...
def test_fast_deep_lane_doc_contains_fast_lane() -> None:
    ...
    assert "Fast Lane" in text, (
        "FAST_DEEP_LANE.md is missing a 'Fast Lane' section."
    )
...
def test_fast_deep_lane_doc_contains_deep_lane() -> None:
    ...
    text = LANE_DOCS.read_text(encoding="utf-8")
    assert "Deep Lane" in text, (
        "FAST_DEEP_LANE.md is missing a 'Deep Lane' section."
    )
...
def test_pytest_fast_ini_exists() -> None:
    ...
    assert PYTEST_FAST_INI.exists(), (
        f"pytest-fast.ini not found at {PYTEST_FAST_INI}."
    )
...
def test_taskfile_contains_test_deep() -> None:
    ...
    assert "test:deep" in text, (
        "Taskfile.yml does not contain a 'test:deep' task. "
        "Required by WL-134 for deep lane opt-in."
    )
