...
def test_wl6911_parse_log_entry_valid_payload() -> None:
    ...
    row = json.dumps({"type": "assistant", "timestamp": "2026-01-10T10:00:00+00:00", "message": {"content": "ok"}})
...
def test_wl6911_parse_log_entry_invalid_timestamp() -> None:
    ...
    row = json.dumps({"type": "assistant", "timestamp": "not-a-time", "message": {"content": "bad"}})
    parsed = summary._parse_log_entry(row, start, end, stats)
...
def test_wl6912_read_log_file_readable(tmp_path: Path) -> None:
    ...
    log_file.write_text(
        json.dumps({"type": "user", "timestamp": "2026-01-12T09:00:00+00:00", "message": {"content": "hello"}}) + "\n",
        encoding="utf-8",
    )
