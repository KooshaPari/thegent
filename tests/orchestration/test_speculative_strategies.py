...
class TestSpeculativeConfig:
    ...
    def test_defaults(self) -> None:
        ...
        assert config.strategy == SpeculativeStrategy.RACE_FIRST
        assert config.timeout_ms == 5000
        assert config.quality_threshold == 0.8
        assert config.cost_budget_usd == 0.01
        assert config.historical_latency_p95_ms == 2000.0
        assert config.historical_quality_avg == 0.85
