...
class TestTaskWorkerPoolWorkerLoop:
    """Tests for TaskWorkerPool worker loop behavior."""
    ...
    def pool(self, tmp_path: Path) -> TaskWorkerPool:
        ...
        with patch("thegent.orchestration.worker_pool.ThegentSettings") as mock:
            mock.return_value.cache_dir = tmp_path
            pool = TaskWorkerPool(queue_dir=tmp_path / "queue", max_workers=1)
            return pool

    @pytest.mark.asyncio
    async def test_worker_processes_task(self, pool: TaskWorkerPool) -> None:
        """Verify worker processes a task from inbox."""
        ...
        async def run_briefly():
            try:
                await asyncio.wait_for(pool._worker_loop(0), timeout=3.0)
        ...
        # Check result exists
        result = pool.get_result("async-task-001", timeout=1)
        if result:
            assert result.exit_code == 0
            assert "hello from worker" in result.stdout

    @pytest.mark.asyncio
    async def test_worker_handles_task_error(self, pool: TaskWorkerPool) -> None:
        """Verify worker handles command that fails."""
        task = TaskRequest(
            id="async-task-002",
        ...
        result = pool.get_result("async-task-002", timeout=1)
        if result:
            assert result.exit_code != 0


class TestTaskWorkerPoolStart:
    """Tests for TaskWorkerPool.start method."""
    ...
    def pool(self, tmp_path: Path) -> TaskWorkerPool:
        ...
        with patch("thegent.orchestration.worker_pool.ThegentSettings") as mock:
            mock.return_value.cache_dir = tmp_path
            return TaskWorkerPool(queue_dir=tmp_path / "queue", max_workers=2)

    @pytest.mark.asyncio
    async def test_start_sets_running(self, pool: TaskWorkerPool) -> None:
        """Verify start sets _running to True."""
        # Create a task that will stop the pool
        pool._running = False

        # Start will set _running and then run workers
        ...
        assert pool._running is False  # Should be stopped now

    @pytest.mark.asyncio
    async def test_start_spawns_workers(self, pool: TaskWorkerPool) -> None:
        """Verify start spawns the correct number of workers."""
        ...
        await start_and_stop()
        # If we got here without hanging, workers were spawned
