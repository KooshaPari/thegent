...
def test_classify_usage_limit() -> None:
...
def test_is_retryable_not_usage_limit() -> None:
    assert is_retryable(RunResult(1, "", "quota exceeded")) is False


def test_is_retryable_gateway() -> None:
...
def test_is_retryable_success_not_retryable() -> None:
    assert is_retryable(RunResult(0, "ok", "")) is False


def test_is_retryable_unknown_provider_not_retryable() -> None:
    # Config/routing errors are not retryable
    assert is_retryable(RunResult(1, "", "unknown provider for model")) is False


def test_usage_limit_error_holds_result() -> None:
...
def test_transient_agent_error_holds_result() -> None:
...
def test_with_retry_returns_last_result_after_exhausted() -> None:
