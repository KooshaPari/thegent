...
def test_wl358_cross_schema_validator_fails_without_evidence_policy(tmp_path: Path) -> None:
    ...
    req.write_text(json.dumps({"id": "FR-TEST-001"}), encoding="utf-8")
    evidence.write_text(json.dumps({"requirement_id": "FR-TEST-001", "kind": "artifact"}), encoding="utf-8")
    attestation.write_text(json.dumps({"subject": []}), encoding="utf-8")
