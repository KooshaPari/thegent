...
def parse_csv(path: pathlib.Path) -> list[dict[str, str]]:
    try:
        ...
        print(f"B95 failed to read CSV input: {exc}", file=sys.stderr)
        raise SystemExit(2)
...
def to_float(raw: str | None, *, context: str, label: str) -> float:
    try:
        ...
        print(
            f"B95 invalid numeric value for {label} ({context}): {raw!r}",
            file=sys.stderr,
        )
        raise SystemExit(2)
...
if a.json:
    try:
        ...
        print(f"B95 invalid JSON input: {exc}", file=sys.stderr)
        raise SystemExit(2)
    if isinstance(payload, dict):
        ...
        print("B95 invalid JSON payload type for --json", file=sys.stderr)
        raise SystemExit(2)
else:
...
for row in rows:
    ...
    passed = to_float(row.get(a.integrity_key), context="row", label=a.integrity_key)
    integrity_ratio = (passed / total) if total > 0 else 1.0
    if integrity_ratio < a.min_integrity_ratio:
        breaches += 1

if breaches > a.max_breaches:
    print("B95 signature preflight integrity gate failed", file=sys.stderr)
    raise SystemExit(2)
