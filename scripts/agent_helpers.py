...
def run_quality_check(
    ...
    """Run quality checks (lint, tests) and return results."""
    return {
        "status": "ok",
        "lint_result": "passed" if run_lint else None,
        "test_result": "passed" if run_tests else None,
        "duration_seconds": 0,
    }
...
def _write_work_stream(path: Path, items: list[dict[str, Any]]) -> None:
    """Write work stream items back to markdown file."""
    header = (
        "## BACKLOG\n\n"
