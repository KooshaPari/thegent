    ...
    for match in matches:
        ...
        if func_match:
            ...
            commands.append({
                ...
                "command": cmd_name,
                "function": func_name,
                "full_path": f"thegent {app_name.replace('_app', '')} {cmd_name}" if app_name != "app" else f"thegent {cmd_name}",
            })
...
def generate_coverage_report() -> dict[str, Any]:
    ...
    return {
        ...
        "missing_tests": [
            full_path for full_path, data in coverage_map.items()
            if not data["has_e2e_test"]
        ],
    }
...
def main():
    ...
    print(f"  2. Implement E2E tests for missing commands")
    print(f"  3. Target: 100% E2E coverage (agent-only requirement)")
    print(f"  4. Run: pytest tests/test_e2e_cli.py -v")
