...
async def test_dependencies():
    ...
    """)
    db.execute_query(f"INSERT OR IGNORE INTO dependencies (item_id, depends_on_item_id) VALUES ('{item_b}', '{item_a}')")
    ...
    db.execute_query(f"UPDATE workstream_items SET status = 'completed', completed_at = '{datetime.now(UTC).isoformat()}' WHERE item_id = '{item_a}'")
    ...
    db.execute_query("INSERT OR REPLACE INTO workstream_items (item_id, title, status, priority) VALUES ('item-C', 'Task C', 'pending', 'P0')")
    db.execute_query("INSERT OR REPLACE INTO workstream_items (item_id, title, status, priority) VALUES ('item-D', 'Task D', 'pending', 'P2')")
