...
THGENT_GH_BRANCH="${THGENT_GH_BRANCH:-main}"
ACT_BRANCH="${ACT_BRANCH:-$THGENT_GH_BRANCH}"
...
RUN_TOKEN="$(date -u +%Y%m%dT%H%M%S)-${RANDOM}"
...
run_dockerless_ci_job() {
  case "$1" in
    ...
    *)
      ...
      echo "Supported jobs: preflight, test, quality, quality-unified, coverage, template-collect-nightly, leak-detection, zig-readiness, integration, pre-commit, pre-push." >&2
      echo "Set ACT_DOCKERLESS=1 with a supported job, or run without ACT_JOB for the full local ci:local lane." >&2
      return 1
      ;;
  esac
}
...
if [[ -n "${THGENT_ACT_EVENT_FILE:-}" ]]; then
  ...
  base_ref="${THGENT_GH_BASE_REF:-${ACT_BRANCH}}"
  ...
  cat > "$event_file" <<EOF_EVENT
{
  ...
  "head_commit": {
    "id": "${head_sha}",
    "message": "Local GH actions simulation",
...
if ! command -v act >/dev/null 2>&1; then
  echo "act is required for GH Actions emulation."
  echo "Install with: brew install act"
  echo "Install with: go install github.com/nektos/act@latest"
  exit 1
fi
...
if ! command -v docker >/dev/null 2>&1 && ! command -v podman >/dev/null 2>&1; then
  echo "act requires Docker or Podman. Install/start one runtime and retry." >&2
...
if command -v docker >/dev/null 2>&1; then
  ...
  echo "Podman is available but not running/reachable. Start Podman and retry local GH Actions emulation." >&2
  exit 1
fi
