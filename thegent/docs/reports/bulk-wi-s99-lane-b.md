### [WL-10580]
**Title:** Preserve hook pipeline by separating before and after handlers
**Source:** [thegent/src/thegent/observability/prometheus.go:360]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10581]
**Title:** Preserve session lifecycle by separating transition and persistence writes
**Source:** [thegent/src/thegent/mcp/server.go:377]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10582]
**Title:** Preserve sync safety by separating detection and application
**Source:** [thegent/src/thegent/commands/dispatch.go:394]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10583]
**Title:** Preserve observability by separating metric producers from transport sink
**Source:** [thegent/src/thegent/integrations/gh_project_sync.py:411]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10584]
**Title:** Preserve runtime health by separating recoverable and terminal failures
**Source:** [thegent/src/thegent/hooks/dispatcher.go:428]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10585]
**Title:** Preserve hook pipeline by separating before and after handlers
**Source:** [thegent/src/thegent/session/state.go:445]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10586]
**Title:** Preserve session lifecycle by separating transition and persistence writes
**Source:** [thegent/src/thegent/automation/workflow.go:462]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10587]
**Title:** Preserve sync safety by separating detection and application
**Source:** [thegent/src/thegent/providers/registry.go:479]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10588]
**Title:** Preserve observability by separating metric producers from transport sink
**Source:** [thegent/src/thegent/queue/storage.go:496]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10589]
**Title:** Preserve runtime health by separating recoverable and terminal failures
**Source:** [thegent/src/thegent/runner/runner.go:513]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.
