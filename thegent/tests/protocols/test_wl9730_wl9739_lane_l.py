...
def test_wl9730_parse_phase_rejects_missing_turn_id() -> None:
    ...
    response, notifications = process_jsonrpc_line_full(
        json.dumps({"jsonrpc": "2.0", "id": "c1", "method": "turn/cancel", "params": {}}).decode()
    )
...
def test_wl9731_success_path_cancels_in_progress_turn() -> None:
    ...
    response, notifications = process_jsonrpc_line_full(
        json.dumps({"jsonrpc": "2.0", "id": "c2", "method": "turn/cancel", "params": {"turn_id": turn_id}}).decode()
    )
...
def test_wl9732_lookup_miss_branch_returns_turn_not_found() -> None:
    ...
    response, notifications = process_jsonrpc_line_full(
        json.dumps({"jsonrpc": "2.0", "id": "c3", "method": "turn/cancel", "params": {"turn_id": "turn-9999"}}).decode()
    )
...
def test_wl9733_dispatch_produces_serialized_turn_projection() -> None:
    ...
    response, _notifications = process_jsonrpc_line_full(
        json.dumps({"jsonrpc": "2.0", "id": "c4", "method": "turn/cancel", "params": {"turn_id": turn_id}}).decode()
    )
...
def test_wl9734_terminal_turn_fails_before_state_mutation() -> None:
    ...
    first, _ = process_jsonrpc_line_full(
        json.dumps({"jsonrpc": "2.0", "id": "c5a", "method": "turn/cancel", "params": {"turn_id": turn_id}}).decode()
    )
    assert first is not None

    second, notifications = process_jsonrpc_line_full(
        json.dumps({"jsonrpc": "2.0", "id": "c5b", "method": "turn/cancel", "params": {"turn_id": turn_id}}).decode()
    )
...
def test_wl9736_recovery_branch_preserves_resolved_approval_state() -> None:
    ...
    session_id = _start_session()
    turn_id = _submit_turn(session_id, requires_approval=True)
    approval_id = SERVER_STATE.turns[turn_id]["approval_id"]
    ...
    response, _notifications = process_jsonrpc_line_full(
        json.dumps({"jsonrpc": "2.0", "id": "c7", "method": "turn/cancel", "params": {"turn_id": turn_id}}).decode()
    )
...
def test_wl9737_requested_approval_is_cancelled_on_turn_cancel() -> None:
    ...
    response, _notifications = process_jsonrpc_line_full(
        json.dumps({"jsonrpc": "2.0", "id": "c8", "method": "turn/cancel", "params": {"turn_id": turn_id}}).decode()
    )
...
def test_wl9738_non_requested_approval_status_is_preserved() -> None:
    ...
    response, _notifications = process_jsonrpc_line_full(
        json.dumps({"jsonrpc": "2.0", "id": "c9", "method": "turn/cancel", "params": {"turn_id": turn_id}}).decode()
    )
...
def test_wl9739_invalid_params_type_fails_before_cancel_execution() -> None:
    ...
    response, notifications = process_jsonrpc_line_full(
        json.dumps({"jsonrpc": "2.0", "id": "c10", "method": "turn/cancel", "params": []}).decode()
    )
