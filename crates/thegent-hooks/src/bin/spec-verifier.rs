...
fn main() -> ExitCode {
    ...
    let fr_path = input.fr_path.unwrap_or_else(|| input.project_dir.join("FUNCTIONAL_REQUIREMENTS.md"));
    let test_path = input.test_path.unwrap_or_else(|| input.project_dir.join("tests"));

    // Read FR requirements
    let fr_content = match fs::read_to_string(&fr_path) {
        Ok(c) => c,
        Err(_) => {
            println!(r#"{{"all_frs":[], "covered_frs":[], "orphan_frs":[], "orphan_tests":[], "coverage":1.0,"exit_code":0}}"#);
    ...
    println!(r#"{{"all_frs":{:?}, "covered_frs":{:?}, "orphan_frs":{:?}, "orphan_tests":{:?}, "coverage":{},"exit_code":{}}}"#, 
        all_frs, covered_count, all_frs.len() - covered_count, test_files.len(), coverage, exit_code);
