    ...
    try:
        with open(file_path, 'r') as f:
            ...
            for title, url in matches:
                links.append({"title": title, "url": url, "source": f"MD: {os.path.basename(file_path)}"})
            
            # Find bare URLs
            bare_urls = re.findall(r'(?<!\()https?://[a-zA-Z0-9./_-]+', content)
            for url in bare_urls:
                if not any(l['url'] == url for l in links):
                    links.append({"title": "Direct Link", "url": url, "source": f"MD: {os.path.basename(file_path)}"})
...
print(f"Extracted {len(unique_md_links)} unique links from markdown files.")
...
with open('docs/research/to-research-queue.md', 'a') as out:
    ...
    for src in sorted(sources.keys()):
        ...
        out.write(f"### {src}\n")
        for item in items:
            out.write(f"- [{item['title']}]({item['url']})\n")
        out.write("\n")
