...
When you run `agileplus research`, the following files are generated in your feature directory:
...
**Location**: All files go in `agileplus/001-feature-name/`
...
2. Run `agileplus research` to generate the mission-specific research artifacts. (Add `--force` only when it is acceptable to overwrite existing drafts.)
...
- `agileplus/<feature>/research.md` explains every major decision with references to evidence.
- `agileplus/<feature>/data-model.md` lists the entities and relationships needed for implementation.
