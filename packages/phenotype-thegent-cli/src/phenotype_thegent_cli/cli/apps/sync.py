...
def _load_bootstrap_sync_module() -> Any:
    spec = importlib.util.spec_from_file_location(
        "phenotype_thegent_bootstrap_sync_workflow_project",
...
def sync_all(
    ...
    from phenotype_thegent_cli.cli.commands.cli_sync import sync_cmd_impl
...
def sync_work_stream_full(
    ...
    # @trace WL-037
    """
    from phenotype_thegent_cli.commands.sync import SyncCommand, SyncOperationStatus
...
def sync_update(
    ...
    from phenotype_thegent_cli.cli.commands.cli_sync import update_cmd_impl
...
def sync_conflicts(
    ...
    from phenotype_thegent_sync.sync.conflicts import render_conflict_surface
    from phenotype_thegent_sync.sync.queue import ConflictQueueStore
...
def sync_freeze(
    ...
    from phenotype_thegent_sync.sync.controller import SyncController
...
def sync_health(
    ...
    from phenotype_thegent_sync.sync.health import ConnectorHealth, render_health_scoreboard
...
def sync_ga_readiness(
    ...
    from phenotype_thegent_sync.sync.ga_readiness import evaluate_ga_readiness
    ...
    if format == "json":
        console.print(json.dumps(payload, option=json.OPT_SORT_KEYS))
    elif format == "rich":
...
def sync_dead_letter_queue(
    ...
    # @trace WL-331
    """
    from phenotype_thegent_cli.commands.sync import SyncCommand
...
def sync_rollback(
    ...
    # @trace WL-185
    """
    from phenotype_thegent_sync.integrations.reflection_rollback import ReflectionRollbackManager
...
def sync_autopilot(
    ...
    from phenotype_thegent_sync.integrations.workstream_autosync import (
