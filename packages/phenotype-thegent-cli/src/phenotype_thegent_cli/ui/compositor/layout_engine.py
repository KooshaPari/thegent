...
class Size:
    ...
    def _fmt_value(self) -> str:
        """Format value, omitting fractional .0 for whole numbers."""
    ...
    def __str__(self) -> str:
        ...
        if self.unit == SizeUnit.AUTO:
            return "auto"
        return f"{self._fmt_value()}{self.unit.value}"
    ...
    def to_textual_css(self) -> str:
        ...
        if self.unit == SizeUnit.PERCENT:
            return f"{self._fmt_value()}%"
        if self.unit == SizeUnit.FRACTION:
            return f"{self._fmt_value()}fr"
...
class LayoutNode:
    ...
    def get_css_for_child(self, index: int) -> str:
        ...
        flex = child.constraints.flex_grow
        flex_str = str(int(flex)) if flex == int(flex) else str(flex)
        return f"{flex_str}fr"
