...
class DoctorRunner:
    ...
    def _check_thegent_dir(self) -> DoctorCheck:
        phenotype_thegent_dir = Path.home() / ".thegent"

        if phenotype_thegent_dir.is_dir():
            return DoctorCheck(
                name="phenotype_thegent_home_dir",
                status="ok",
                message=f"{phenotype_thegent_dir} exists",
            )

        def _fix() -> str:
            phenotype_thegent_dir.mkdir(parents=True, exist_ok=True)
            return f"Created {phenotype_thegent_dir}"

        return DoctorCheck(
            name="phenotype_thegent_home_dir",
            status="error",
            message=f"{phenotype_thegent_dir} does not exist",
            fixable=True,
            _fix_fn=_fix,
        )
    ...
    def _check_thegent_sessions_dir(self) -> DoctorCheck:
        ...
        if sessions_dir.is_dir():
            return DoctorCheck(
                name="phenotype_thegent_sessions_dir",
                status="ok",
    ...
    def _check_thegent_dir_writable(self) -> DoctorCheck:
        ...
        if not phenotype_thegent_dir.exists():
            ...
            return DoctorCheck(
                name="phenotype_thegent_dir_writable",
                status="ok",
                message="~/.thegent/ not yet created (will be fixed by phenotype_thegent_home_dir check)",
            )

        if os.access(phenotype_thegent_dir, os.W_OK):
            return DoctorCheck(
                name="phenotype_thegent_dir_writable",
                status="ok",
                message=f"{phenotype_thegent_dir} is writable",
            )

        def _fix() -> str:

            shim_run(["chmod", "700", str(phenotype_thegent_dir)], check=True, timeout=5)
            return f"chmod 700 {phenotype_thegent_dir}"

        return DoctorCheck(
            name="phenotype_thegent_dir_writable",
            status="error",
            message=f"{phenotype_thegent_dir} is not writable; agent operations will fail",
            fixable=True,
            _fix_fn=_fix,
        )
