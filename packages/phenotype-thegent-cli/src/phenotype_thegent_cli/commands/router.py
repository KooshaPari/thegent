...
if TYPE_CHECKING:
    ...
    from phenotype_thegent_core.utils.routing_impl.route_executor import RouterStatus
...
def router_status(
    ...
    from phenotype_thegent_core.config import ThegentSettings
    from phenotype_thegent_core.utils.routing_impl.route_executor import read_routing_audit
...
def _build_status_from_audit(
    ...
    from phenotype_thegent_core.utils.routing_impl.route_executor import AgentRoutingState, RouterStatus
    ...
    for rec in records:
        ...
        if provider == "lifecycle":
            ...
            state.phenotype_thegent_decisions += 1
    ...
    tg_total = sum(a.phenotype_thegent_decisions for a in agents)
    lifecycle_pct = (lc_total / total * 100.0) if total > 0 else 0.0
    phenotype_thegent_pct = (tg_total / total * 100.0) if total > 0 else 0.0

    quorum: str | None = None
    if providers:
        phenotype_thegent_votes = sum(1 for a in agents if a.phenotype_thegent_decisions > a.lifecycle_decisions)
        quorum = "TheGent" if phenotype_thegent_votes >= len(agents) - phenotype_thegent_votes else "Lifecycle"
    ...
    return RouterStatus(
        ...
        phenotype_thegent_pct=phenotype_thegent_pct,
    )
