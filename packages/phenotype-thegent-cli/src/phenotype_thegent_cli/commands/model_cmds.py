...
from phenotype_thegent_cli.commands.model_cmds_agents_helpers import render_agents_table, render_droids_table
from phenotype_thegent_cli.commands.model_cmds_catalog_helpers import (
    emit_by_model_view,
    emit_contract_view,
    provider_sequence,
    run_provider_listings,
)
from phenotype_thegent_cli.commands.model_cmds_metrics_helpers import (
    build_index_data,
...
from phenotype_thegent_cli.commands.model_cmds_route_helpers import build_available_routes, build_resolved_route
from phenotype_thegent_cli.commands.model_cmds_setup_helpers import (
    build_provider_list,
    configure_providers,
    set_env_line,
)
...
def speed_index_cmd(
    ...
    """
    from phenotype_thegent_core.models.speed_values import (
...
def quality_index_cmd(
    ...
    """
    from phenotype_thegent_core.models.quality_values import (
...
def metrics_cmd(
    ...
    from phenotype_thegent_core.models.cost_values import get_model_provider_costs
    from phenotype_thegent_core.models.quality_values import (
        get_model_provider_quality_indices,
        invalidate_quality_index_cache,
    )
...
def resolve_model_route_cmd(
    ...
    from phenotype_thegent_core.models import (
        ModelCatalog,
        normalize_model_id,
        normalize_route_policy,
        resolve_route_contract,
    )
    from phenotype_thegent_core.models.quality_values import get_model_provider_quality_indices
    from phenotype_thegent_core.models.speed_values import get_model_provider_speed_indices
...
def list_model_contract_schema_cmd() -> None:
    ...
    from phenotype_thegent_core.models import route_contract
...
def _list_cursor_api_models() -> None:
    ...
    from phenotype_thegent_core.models.scrapers import scrape_cursor_api
...
def setup_cmd(
    ...
    from phenotype_thegent_agents.cliproxy_manager import (
        _LOGIN_FLAGS,
    ...
    from phenotype_thegent_core.infra import yaml_dump, yaml_load
    ...
    if harness or full:
        ...
        try:
            from phenotype_thegent_cli.install import setup_harness
    ...
    if full:
        ...
        try:
            from phenotype_thegent_cli.install import run_install
        ...
        try:
            from phenotype_thegent_cli.git_lock_manage import lock_cleanup_install, lock_cleanup_start
    ...
    if should_delegate_setup:
        from phenotype_thegent_agents.cliproxy_manager import _binary_available, _resolve_binary
    ...
    try:
        from phenotype_thegent_agents.cliproxy_manager import _ensure_config
    ...
    if links:
        ...
        try:
            from phenotype_thegent_cli.clode_main import install_links as clode_install_links

            clode_install_links(bin_dir=bin_dir, force=True)
        except Exception as e:
            console.print(f"[red]Clode links: {e}[/red]")
        try:
            from phenotype_thegent_cli.dex_main import install_links as dex_install_links
    ...
    if hooks:
        ...
        try:
            from phenotype_thegent_cli.install import setup_hooks
    ...
    if skills:
        ...
        try:
            from phenotype_thegent_cli.install import setup_skills
    ...
    if wizard:
        ...
        if Confirm.ask(
            ...
            from phenotype_thegent_cli.install import run_wizard
        ...
        if Confirm.ask(
            ...
            try:
                from phenotype_thegent_protocols.mcp.manage import remove_playwright_from_client
...
def rules_sync_cmd(
    ...
    from phenotype_thegent_cli.commands.impl import rules_sync_impl
