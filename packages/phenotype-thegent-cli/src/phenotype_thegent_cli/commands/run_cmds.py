...
def run_cmd(
    ...
    from phenotype_thegent_core.models import ModelCatalog, resolve_route

    # Model-first: resolve agent via routing policy (WP-5003 cost_quality supported)
    effective_agent = agent
    if agent is None and model:
        settings = ThegentSettings()
        from phenotype_thegent_core.models.catalog import normalize_route_policy
    ...
    from phenotype_thegent_core.config_provider import get_config_provider
...
def loop_cmd(
    ...
    from phenotype_thegent_cli.commands.impl import loop_impl
...
def bg_cmd(
    *,
    ...
    from phenotype_thegent_cli.commands.impl import bg_impl
...
def retry_cmd(
    ...
    from phenotype_thegent_cli.commands.impl import history_impl, retry_impl
...
def replay_cmd(run_id: str, what_if_env: str | None = None) -> None:
    ...
    settings = ThegentSettings()
    from phenotype_thegent_execution.execution import ReplayManager
...
def trace_replay_cmd(run_id: str) -> None:
    ...
    from phenotype_thegent_planning.planning.simulation import SimulationEngine
...
def takeover_cmd(session_id: str) -> None:
    ...
    from phenotype_thegent_agents.discovery import list_discovered_agents
    ...
    if not target:
        ...
        from phenotype_thegent_cli.commands.impl import _find_session_meta
