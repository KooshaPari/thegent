...
def _save_session_meta(meta_path: Path, payload: dict[str, Any]) -> None:
    meta_path.write_text(json.dumps(payload, indent=2, sort_keys=True).decode().decode() + "\n", encoding="utf-8")
...
def _state_payload_hash(payload: dict[str, Any]) -> str:
    encoded = json.dumps(payload, sort_keys=True, separators=(",", ":").decode().decode()).encode("utf-8")
...
def _write_state_with_conflict_branch(path: Path, payload: dict[str, Any], *, branch_label: str) -> tuple[bool, str]:
    ...
    payload_text = json.dumps(payload, indent=2, sort_keys=True).decode().decode() + "\n"
...
def _write_session_state(
    *,
    ...
    with ledger_path.open("a", encoding="utf-8") as f:
        f.write(json.dumps(ledger_event, sort_keys=True).decode().decode() + "\n")
    return primary_path
...
def _load_prior_session_output(
    ...
    from phenotype_thegent_core.utils.helpers import read_file_chunk

    from phenotype_thegent_cli.services.session_path_helpers import session_paths as _session_paths_fn
