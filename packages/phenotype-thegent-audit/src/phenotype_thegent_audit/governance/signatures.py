...
BKM-03: When THGENT_USE_NATIVE_CRYPTO=1, uses phenotype_thegent_crypto Rust extension
...
def _get_native_crypto() -> Any:
    """Lazy import of phenotype_thegent_crypto native extension. Returns None if unavailable."""
    global _thegent_crypto
    if _thegent_crypto is not None:
        return _thegent_crypto
    if not ThegentSettings().use_native_crypto:
        return None
    spec = importlib.util.find_spec("phenotype_thegent_crypto.phenotype_thegent_crypto")
...
def _canonical_json(data: dict[str, Any]) -> bytes:
    ...
    return orjson.dumps(data, non_SORT_KEYS).decode()
