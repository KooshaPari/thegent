...
class TeamManager:
    ...
    def create_team(self, name: str, leader: str, teammates: list[str]) -> str:
        ...
        (self.teams_dir / f"{team_id}.json").write_text(json.dumps(team_meta).decode(), encoding="utf-8")
    ...
    def add_task(self, team_id: str, title: str, description: str, dependencies: list[str] | None = None) -> str:
        ...
        with (self.teams_dir / f"{team_id}_tasks.jsonl").open("a", encoding="utf-8") as f:
            f.write(json.dumps(task).decode() + "\n")
        return task_id
        ...
        with tasks_file.open("r", encoding="utf-8") as f:
            for line in f:
                try:
                    ...
                    # Malformed task line — skip to keep remaining tasks accessible.
                    _ = e
                    continue
        return tasks
