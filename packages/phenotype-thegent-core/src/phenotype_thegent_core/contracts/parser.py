...
BKM-02: When THGENT_USE_NATIVE_PARSER=1, uses phenotype_thegent_parser Rust extension
...
def _get_native_parser() -> Any:
    """Lazy import of phenotype_thegent_parser native extension. Returns None if unavailable."""
    global _thegent_parser
    if _thegent_parser is not None:
        return _thegent_parser
    if not ThegentSettings().use_native_parser:
        return None
    for mod_path in ("phenotype_thegent_parser.phenotype_thegent_parser", "phenotype_thegent_parser"):
...
def extract_tags(text: str, tags: list[str] | None = None) -> dict[str, str]:
    ...
    BKM-02: Uses phenotype_thegent_parser Rust extension when THGENT_USE_NATIVE_PARSER=1;
