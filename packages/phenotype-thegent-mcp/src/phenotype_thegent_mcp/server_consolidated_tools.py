...
def register_consolidated_tools(*, mcp: FastMCP, logger: logging.Logger) -> tuple[object, ...]:
    ...
    # -------------------------------------------------------------------------
    # phenotype_thegent_web: Unified web research (ddg, reddit, scrape, deep)
    # -------------------------------------------------------------------------
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def phenotype_thegent_web(
        ...
        """
        from phenotype_thegent_protocols.mcp.server.tools_research import (
            phenotype_thegent_ddg_search_impl,
            phenotype_thegent_scrape_url_impl,
            phenotype_thegent_reddit_search_impl,
            phenotype_thegent_deep_research_impl,
            phenotype_thegent_suggest_prompt_impl,
        )

        if action == "search":
            return await phenotype_thegent_ddg_search_impl(query=query, num_results=num_results, ctx=ctx)
        if action == "scrape":
            return await phenotype_thegent_scrape_url_impl(url=url, use_playwright=True, ctx=ctx)
        if action == "reddit":
            return phenotype_thegent_reddit_search_impl(query=query, num_results=num_results)
        if action == "deep":
            return phenotype_thegent_deep_research_impl(query=query, subreddits=None)
        if action == "suggest":
            return await phenotype_thegent_suggest_prompt_impl(raw_prompt=query, ctx=ctx, logger=logger)
        return ToolResult(content=f"Unknown action: {action}")

    # -------------------------------------------------------------------------
    # phenotype_thegent_queue: Unified queue operations
    # -------------------------------------------------------------------------
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": False})
    async def phenotype_thegent_queue(
        ...
        """
        from phenotype_thegent_protocols.mcp.server.tools_queue import (
            queue_add_impl,
            queue_list_impl,
            queue_done_impl,
            queue_claim_impl,
    ...
    # -------------------------------------------------------------------------
    # phenotype_thegent_session: Unified session management
    # -------------------------------------------------------------------------
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def phenotype_thegent_session(
        ...
        """
        from phenotype_thegent_protocols.mcp.server.tools_sessions import (
            session_list_impl,
            session_show_impl,
            session_logs_impl,
            session_send_impl,
        ...
        from phenotype_thegent_cli.cli.commands.session_ops_impl import ps_impl, logs_impl
        from phenotype_thegent_cli.cli.commands.session_control_impl import session_send_impl as raw_send_impl
    ...
    # -------------------------------------------------------------------------
    # phenotype_thegent_workstream: Unified workstream operations
    # -------------------------------------------------------------------------
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": False})
    async def phenotype_thegent_workstream(
    ...
    return (
        phenotype_thegent_web,
        phenotype_thegent_queue,
        phenotype_thegent_session,
        phenotype_thegent_workstream,
    )
