...
def register_ops_tools(
    *,
    ...
    def phenotype_thegent_list_operations(operation: str | None = None) -> ToolResult:
        """List universal operation taxonomy: orchestrate, govern, recover, observe, plan."""
        return server_tools_catalog.phenotype_thegent_list_operations_impl(
            operation=operation,
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_list_modes(mode: str | None = None) -> ToolResult:
        """List multi-agent orchestration modes (G-KD-04)."""
        return server_tools_catalog.phenotype_thegent_list_modes_impl(
            mode=mode,
            stable_json_impl=stable_json,
            error_result_impl=error_result,
        )
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_suggest_mode(
        risk: str = "medium",
        urgency: str = "normal",
        confidence: float = 0.8,
    ) -> ToolResult:
        """WP-Y1: Suggest multi-agent mode based on risk, urgency, confidence (FR-032)."""
        return server_tools_catalog.phenotype_thegent_suggest_mode_impl(
            risk=risk,
            urgency=urgency,
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_list_agents() -> ToolResult:
        """List available canonical agents for task execution."""
        return server_tools_catalog.phenotype_thegent_list_agents_impl(
            list_agents_impl=list_agents_impl,
        )

    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_list_models(
        provider: str | None = None,
        include_contract: bool = False,
        by_model: bool = False,
    ) -> ToolResult:
        """List available AI models and their provider mappings."""
        return server_tools_catalog.phenotype_thegent_list_models_impl(
            provider=provider,
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_resolve_model_route(
        model: str,
        provider: str | None = None,
        policy: str = "prefer_direct",
    ) -> ToolResult:
        """Resolve a model to a concrete routing target."""
        return server_tools_catalog.phenotype_thegent_resolve_model_route_impl(
            model=model,
            provider=provider,
            policy=policy,
            error_result_impl=error_result,
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_session_contracts(
        owner: str | None = None,
        all: bool = False,
        ...
        return server_tools_contract_observe.phenotype_thegent_session_contracts_impl(
            owner=owner,
            all=all,
    ...
    def phenotype_thegent_session_contract_health_trend(
        ...
        return server_tools_contract_observe.phenotype_thegent_session_contract_health_trend_impl(
    ...
    def phenotype_thegent_observe_summary(
        ...
        return server_tools_contract_observe.phenotype_thegent_observe_summary_impl(
            limit=limit,
    ...
    return (
        ...
        phenotype_thegent_session_contract_health_gate,
        phenotype_thegent_session_contract_health_report,
