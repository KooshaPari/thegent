...
from phenotype_thegent_cli.cli.commands.impl import (
    ELICIT_CWD_MSG,
    ELICIT_OWNER_MSG,
    _coerce_issue_types,
    _default_owner_tag,
...
from phenotype_thegent_cli.ide.auto_init import auto_init_on_startup
from phenotype_thegent_protocols.mcp import server_bootstrap as _server_bootstrap
from phenotype_thegent_protocols.mcp import server_cache_elicitation_response as _cache_elicitation_response_shared
from phenotype_thegent_protocols.mcp import server_catalog_tools as _server_tools_catalog
from phenotype_thegent_protocols.mcp import server_consolidated_tools as _server_consolidated_tools
from phenotype_thegent_protocols.mcp import server_control_tools as _server_control_tools
from phenotype_thegent_protocols.mcp import server_create_elicitation_cache as _create_elicitation_cache_shared
from phenotype_thegent_protocols.mcp import server_default_cwd_from_context as _default_cwd_from_context_shared
from phenotype_thegent_protocols.mcp import server_default_owner_from_context as _default_owner_from_context_shared
from phenotype_thegent_protocols.mcp import server_elicitation_cache_key as _cache_elicitation_key_shared
from phenotype_thegent_protocols.mcp import server_error_result as _shared_error_result
from phenotype_thegent_protocols.mcp import server_execution_tools as _server_execution_tools
from phenotype_thegent_protocols.mcp import server_get_cached_elicitation as _get_cached_elicitation_shared
from phenotype_thegent_protocols.mcp import server_journal_tools as _server_journal_tools
from phenotype_thegent_protocols.mcp import server_load_module as _load_server_module_shared
from phenotype_thegent_protocols.mcp import server_ops_tools as _server_ops_tools
from phenotype_thegent_protocols.mcp import server_optional_tools as _server_optional_tools
from phenotype_thegent_protocols.mcp import server_planning_tools as _server_planning_tools
from phenotype_thegent_protocols.mcp import server_research_tools as _server_research_tools
from phenotype_thegent_protocols.mcp import server_resource_routes as _server_resource_routes
from phenotype_thegent_protocols.mcp import server_resolve_cwd_elicitation as _resolve_cwd_elicitation_shared
from phenotype_thegent_protocols.mcp import server_resolve_owner_elicitation as _resolve_owner_elicitation_shared
from phenotype_thegent_protocols.mcp import server_runtime_entry as _server_runtime_entry
from phenotype_thegent_protocols.mcp import server_stable_json as _shared_stable_json
from phenotype_thegent_protocols.mcp import server_terminal_tools as _server_terminal_tools
from phenotype_thegent_protocols.mcp.server_dispatch_helpers import (
...
from phenotype_thegent_protocols.mcp.server_middleware import setup_middleware as _setup_middleware
...
from phenotype_thegent_protocols.mcp.server_resources import (
...
from phenotype_thegent_protocols.mcp.server_runtime_helpers import (
    create_event_store,
    create_http_app,
    health_response,
    lifespan_proxy,
...
from phenotype_thegent_protocols.mcp.server_tool_loader import (
...
from phenotype_thegent_core.output_parser import OUTPUT_PARSER_SCHEMA_VERSION
...
phenotype_thegent_lifespan = _server_bootstrap.build_lifespan(
...
mcp = FastMCP("thegent", lifespan=phenotype_thegent_lifespan)
app = mcp
...
(phenotype_thegent_list_droids, phenotype_thegent_list_skills, phenotype_thegent_activate_skill, phenotype_thegent_terminal_route, phenotype_thegent_macos_run_script) = (
...
(phenotype_thegent_run_agent, phenotype_thegent_create_wbs, phenotype_thegent_bg_task, phenotype_thegent_govern_vet, phenotype_thegent_handoff) = (
...
(phenotype_thegent_harness_interact, phenotype_thegent_harness_list_actions, phenotype_thegent_harness_get_command, phenotype_thegent_harness_register_host) = (
    _server_tools_harness.register_harness_tools(mcp=mcp, server_tools_harness=_server_tools_harness)
)
(
    phenotype_thegent_register_tool,
    phenotype_thegent_complete_tool_call,
    phenotype_thegent_list_dynamic_tools,
...
(
    ...
    phenotype_thegent_session_contracts,
    _ops_gate_obj,
    _ops_report_obj,
    _ops_trend_obj,
    _ops_observe_obj,
    ...
    _server_ops_tools.register_ops_tools(
        ...
        session_contract_health_report_impl=session_contract_health_report_impl,
        session_contract_health_trend_impl=session_contract_health_trend_impl,
        session_contract_health_gate_helper=phenotype_thegent_session_contract_health_gate_helper,
        session_contract_health_report_helper=phenotype_thegent_session_contract_health_report_helper,
...
def phenotype_thegent_session_contract_health_trend(
    ...
    return _server_tools_contract_observe.phenotype_thegent_session_contract_health_trend_impl(
...
def phenotype_thegent_observe_summary(
    ...
    return _server_tools_contract_observe.phenotype_thegent_observe_summary_impl(
        limit=limit,
...
(
    ...
    phenotype_thegent_negotiate_contract,
    phenotype_thegent_run,
    phenotype_thegent_loop,
    phenotype_thegent_loop_takeover,
    phenotype_thegent_loop_stop,
...
(
    ...
    phenotype_thegent_workstream_claim,
    phenotype_thegent_lsp_diagnostics,
    phenotype_thegent_lsp_symbol_lookup,
    phenotype_thegent_lsp_hover,
    phenotype_thegent_workstream_complete,
...
(phenotype_thegent_ddg_search, phenotype_thegent_reddit_search, phenotype_thegent_scrape_url, phenotype_thegent_deep_research, phenotype_thegent_suggest_prompt) = (
    _server_research_tools.register_research_tools(mcp=mcp, server_tools_research=_server_tools_research, logger=_log)
)
(phenotype_thegent_web, phenotype_thegent_queue, phenotype_thegent_session, phenotype_thegent_workstream) = (
    _server_consolidated_tools.register_consolidated_tools(mcp=mcp, logger=_log)
)
(phenotype_thegent_handoff_list, phenotype_thegent_handoff_show, phenotype_thegent_handoff_confirm, phenotype_thegent_queue_list, phenotype_thegent_queue_claim) = (
...
(phenotype_thegent_queue_done, phenotype_thegent_queue_add, phenotype_thegent_queue_edit, phenotype_thegent_queue_release, phenotype_thegent_queue_extend_lease) = (
...
try:
    from phenotype_thegent_protocols.mcp.task_registry import get_task_registry as _get_task_registry

    _task_reg = _get_task_registry()

    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_task_status(task_id: str) -> ToolResult:
        result = _task_reg.status(task_id)
        return ToolResult(
            content=json.dumps(result).decode().decode(), structured_content=result, meta={"execution_time_ms": 0}
        )

    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_task_cancel(task_id: str) -> ToolResult:
        result = _task_reg.cancel(task_id)
        return ToolResult(
            content=json.dumps(result).decode().decode(), structured_content=result, meta={"execution_time_ms": 0}
        )

    _log.info("task mode tools registered: phenotype_thegent_task_status, phenotype_thegent_task_cancel")
...
(health, _get_event_store, phenotype_thegent_acp_invoke, http_app, http_app_factory, run) = (
