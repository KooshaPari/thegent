...
def register_terminal_tools(
    *,
    ...
    def phenotype_thegent_terminal_list(all: bool = False) -> ToolResult:
        """
        ...
        """
        from phenotype_thegent_skills.skills.terminal import is_claude_code_pane, list_tmux_panes
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_terminal_inspect(pane_id: str, last_lines: int = 50) -> ToolResult:
        """
        Capture the content of a terminal pane.
        """
        from phenotype_thegent_skills.skills.terminal import capture_tmux_pane
    ...
    def phenotype_thegent_terminal_send(pane_id: str, text: str, enter: bool = True) -> ToolResult:
        """
        Send text/keys to a terminal pane.
        """
        from phenotype_thegent_skills.skills.terminal import send_to_tmux_pane

        start_time = time.perf_counter()
        success = send_to_tmux_pane(pane_id, text, enter=enter)
        elapsed_ms = int((time.perf_counter() - start_time) * 1000)
        return ToolResult(
            content=json.dumps({"success": success}).decode().decode(),
            structured_content={"success": success},
            meta={"execution_time_ms": elapsed_ms},
        )

    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_terminal_attach(pane_id: str) -> ToolResult:
        """
        Get instructions to attach to a terminal session.
        """
        from phenotype_thegent_skills.skills.terminal import list_tmux_panes

        return server_tools_terminal.phenotype_thegent_terminal_attach_impl(
            pane_id=pane_id,
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_workstream_claim(item_id: str, agent_id: str) -> ToolResult:
        """
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_lsp_diagnostics(file_path: str) -> ToolResult:
        """WL-109: return normalized LSP diagnostics for a file."""
        from phenotype_thegent_protocols.mcp.lsp_tools import lsp_diagnostics
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_lsp_symbol_lookup(symbol_name: str, file_path: str | None = None) -> ToolResult:
        """WL-109: lookup a symbol through the LSP adapter."""
        from phenotype_thegent_protocols.mcp.lsp_tools import lsp_symbol_lookup
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_lsp_hover(file_path: str, line: int, character: int) -> ToolResult:
        """WL-109: return hover information for a source position."""
        from phenotype_thegent_protocols.mcp.lsp_tools import lsp_hover
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_workstream_complete(item_id: str, agent_id: str) -> ToolResult:
        """
