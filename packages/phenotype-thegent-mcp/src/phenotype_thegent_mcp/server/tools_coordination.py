...
def phenotype_thegent_wait_impl(
    *,
    session_id: str,
    timeout: int | None,
    logger: Logger,
    ...
    logger.info("phenotype_thegent_wait session_id=%s timeout=%s", session_id, timeout)
    ...
    if result.get("auto_timeout"):
        message = (
            ...
            f"**Action Required**: Call `phenotype_thegent_wait(session_id='{session_id}', timeout={timeout})` again "
...
def phenotype_thegent_inbox_list_impl(
    *,
    owner: str | None,
    agent: str | None,
    event_type: str | None,
...
def phenotype_thegent_inbox_wait_impl(
    *,
    owner: str | None,
    agent: str | None,
    event_type: str | None,
    ...
    logger.info(
        "phenotype_thegent_inbox_wait owner=%s agent=%s event_type=%s poll=%.1f timeout=%.1f",
        owner,
        agent,
        event_type,
        poll_interval,
    ...
    if isinstance(result, dict) and result.get("auto_timeout"):
        message = (
            ...
            f"**⚠️ CRITICAL: DO NOT TERMINATE THIS CHAT**\n\n"
            f"**Action Required**: Call `phenotype_thegent_inbox_wait(owner={owner}, agent={agent}, event_type={event_type}, "
...
def phenotype_thegent_stop_impl(
    *,
    session_id: str,
    force: bool,
    logger: Logger,
    ...
    logger.info("phenotype_thegent_stop session_id=%s force=%s", session_id, force)
    start_time = time.perf_counter()
...
def phenotype_thegent_pause_impl(
    *,
    session_id: str,
    reason: str,
    logger: Logger,
    ...
    logger.info("phenotype_thegent_pause session_id=%s", session_id)
...
def phenotype_thegent_resume_impl(
    *,
    session_id: str,
    logger: Logger,
    settings_factory: type[ThegentSettings],
) -> ToolResult:
    logger.info("phenotype_thegent_resume session_id=%s", session_id)
    from phenotype_thegent_execution.execution import RunRegistry
...
def phenotype_thegent_continuity_snapshot_impl(
    *,
    owner: str,
    run_ids: list[str],
