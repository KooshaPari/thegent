...
from phenotype_thegent_core.memory.seed_detector import SeedDetector, SeedSource
from phenotype_thegent_core.memory.seed_storage import SeedStorage
...
def register_seed_tools(mcp: "FastMCP") -> None:
    ...
    async def phenotype_thegent_seed_detect(
        text: str,
        ...
        if not text or not isinstance(text, str):
            return ToolResult(
                content=json.dumps({"error": "Invalid input", "seeds": [], "count": 0}).decode().decode(),
                structured_content={"error": "Invalid input", "seeds": [], "count": 0},
                meta={"execution_time_ms": 0},
            )

        try:
            await _ctx_info(ctx, f"phenotype_thegent_seed_detect source={source} use_llm={use_llm} text_len={len(text)}")
            ...
            await _ctx_info(ctx, f"phenotype_thegent_seed_detect found {len(seeds)} seed(s)")
            ...
            await _ctx_warning(ctx, f"phenotype_thegent_seed_detect error: {e}")
            _log.exception("Seed detection error")
            return ToolResult(
                content=json.dumps({"error": str(e).decode().decode(), "seeds": [], "count": 0}),
                structured_content={"error": str(e), "seeds": [], "count": 0},
                meta={"execution_time_ms": 0},
            )

    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": False})
    async def phenotype_thegent_seed_store(
        text: str,
        source: str = "manual",
        ...
        if not text or not isinstance(text, str):
            return ToolResult(
                content=json.dumps({"error": "Invalid input", "seed": None}).decode().decode(),
                structured_content={"error": "Invalid input", "seed": None},
                meta={"execution_time_ms": 0},
            )

        try:
            await _ctx_info(ctx, f"phenotype_thegent_seed_store source={source} confidence={confidence}")
            ...
            if detected_seeds:
                ...
                from phenotype_thegent_core.memory.seed_detector import Seed
            ...
            await _ctx_info(ctx, f"phenotype_thegent_seed_store stored seed_id={seed_id}")
            result = {
                "seed_id": seed_id,
                "stored": True,
                "seed": seed.to_dict(),
            ...
            await _ctx_warning(ctx, f"phenotype_thegent_seed_store error: {e}")
            _log.exception("Seed storage error")
            return ToolResult(
                content=json.dumps({"error": str(e).decode().decode(), "seed": None}),
                structured_content={"error": str(e), "seed": None},
                meta={"execution_time_ms": 0},
            )

    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def phenotype_thegent_seed_list(
        status: str = "new",
        tag: str | None = None,
        ...
        try:
            await _ctx_info(ctx, f"phenotype_thegent_seed_list status={status} tag={tag} source={source}")
            root = _resolve_cwd(Path(cd) if cd else None)
            if not root:
                return ToolResult(
                    content=json.dumps({"error": "No project root", "seeds": []}).decode().decode(),
            ...
            total = len(seeds)
            await _ctx_info(ctx, f"phenotype_thegent_seed_list loaded {total} seed(s) before filtering")
            ...
            await _ctx_warning(ctx, f"phenotype_thegent_seed_list error: {e}")
            _log.exception("Seed listing error")
            return ToolResult(
                content=json.dumps({"error": str(e).decode().decode(), "seeds": []}),
                structured_content={"error": str(e), "seeds": []},
                meta={"execution_time_ms": 0},
            )

    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": False})
    def phenotype_thegent_seed_update(
        seed_id: str,
        ...
        try:
            ...
            if not root:
                return ToolResult(
                    content=json.dumps({"error": "No project root", "updated": False}).decode().decode(),
            ...
            return ToolResult(
                content=json.dumps({"error": str(e).decode().decode(), "updated": False}),
                structured_content={"error": str(e), "updated": False},
                meta={"execution_time_ms": 0},
            )

    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_seed_export(
        cd: str | None = None,
    ) -> ToolResult:
        """
        ...
        try:
            ...
            if not root:
                return ToolResult(
                    content=json.dumps({"error": "No project root", "markdown": None}).decode().decode(),
            ...
            return ToolResult(
                content=json.dumps({"error": str(e).decode().decode(), "markdown": None}),
                structured_content={"error": str(e), "markdown": None},
                meta={"execution_time_ms": 0},
            )

    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_seed_stats(
        cd: str | None = None,
    ) -> ToolResult:
        """
        ...
        try:
            ...
            if not root:
                return ToolResult(
                    content=json.dumps({"error": "No project root", "stats": {}}).decode().decode(),
            ...
            return ToolResult(
                content=json.dumps({"error": str(e).decode().decode(), "stats": {}}),
                structured_content={"error": str(e), "stats": {}},
                meta={"execution_time_ms": 0},
            )

    _ = (
        phenotype_thegent_seed_detect,
        phenotype_thegent_seed_store,
        phenotype_thegent_seed_list,
        phenotype_thegent_seed_update,
        phenotype_thegent_seed_export,
        phenotype_thegent_seed_stats,
    )
