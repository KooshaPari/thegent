...
from phenotype_thegent_cli.cli.commands.impl import _resolve_cwd
...
def register_modes(mcp: "FastMCP") -> None:
    ...
    def phenotype_thegent_plan_status(cd: str | None = None) -> ToolResult:
        """
        ...
        if not root:
            return ToolResult(
                content=json.dumps({"error": "No project root", "remediation": "Set cwd or cd"}).decode().decode(),
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_plan_get(plan_id: str | None = None, cd: str | None = None) -> ToolResult:
        """
        ...
        if not root:
            return ToolResult(
                content=json.dumps({"error": "No project root"}).decode().decode(),
        ...
        if path.is_dir():
            ...
            latest = max(plans, key=lambda p: p.stat().st_mtime) if plans else None
            path = latest or path
        from phenotype_thegent_core.utils.helpers import read_file_optimized

        content = read_file_optimized(path)
        if content is None:
            return ToolResult(
                content=json.dumps({"error": "Failed to read plan", "path": str(path).decode().decode()}),
    ...
    def phenotype_thegent_plan_save(
        content: str,
        ...
        elapsed = int((time.perf_counter() - start) * 1000)
        return ToolResult(
            content=json.dumps({"path": str(path).decode().decode(), "saved": True}),
            structured_content={"path": str(path), "saved": True},
            meta={"execution_time_ms": elapsed},
        )

    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_plan_approve(
        plan_id: str,
        cd: str | None = None,
    ) -> ToolResult:
        """
        ...
        elapsed = int((time.perf_counter() - start) * 1000)
        return ToolResult(
            content=json.dumps({"plan_id": plan_id, "approved": True, "marker": str(marker).decode().decode()}),
            structured_content={"plan_id": plan_id, "approved": True, "marker": str(marker)},
            meta={"execution_time_ms": elapsed},
        )

    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_plan_create(
        prompt: str,
        plan_id: str | None = None,
        ...
        if brief_path:
            ...
            from phenotype_thegent_core.utils.helpers import safe_read_file
...
"""
        ...
        elapsed = int((time.perf_counter() - start) * 1000)
        return ToolResult(
            content=json.dumps({"path": str(path).decode().decode(), "plan_id": name, "created": True}),
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_protocol_list(cd: str | None = None) -> ToolResult:
        """
        ...
        if not root:
            return ToolResult(
                content=json.dumps({"error": "No project root", "protocols": []}).decode().decode(),
        ...
        elapsed = int((time.perf_counter() - start) * 1000)
        return ToolResult(
            content=json.dumps({"protocols": protocols}).decode().decode(),
            structured_content={"protocols": protocols},
            meta={"execution_time_ms": elapsed},
        )

    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_protocol_get(
        mode: str | None = None,
        name: str | None = None,
        cd: str | None = None,
    ) -> ToolResult:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_discussion_finalize(
        brief_content: str,
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_research_finalize(
        report_content: str,
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_validation_report(
        cd: str | None = None,
        ...
        if not root:
            return ToolResult(
                content=json.dumps({"error": "No project root", "report": None}).decode().decode(),
        ...
        elapsed = int((time.perf_counter() - start) * 1000)
        return ToolResult(
            content=json.dumps({"path": str(latest).decode().decode() if latest else None, "content": content}),
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_dag_ready(cd: str | None = None) -> ToolResult:
        """
        List DAG task IDs that are ready (pending with all deps Union[done, cancelled]|skipped).
        Use before phenotype_thegent_dag_run to see what can be spawned.
        """
        start = time.perf_counter()
        from phenotype_thegent_cli.cli.commands.impl import dag_ready_impl
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    async def phenotype_thegent_dag_run(
        cd: str | None = None,
        dry_run: bool = False,
        task: str | None = None,
        ...
        """
        Spawn agents for ready DAG tasks. Use phenotype_thegent_dag_ready first to see ready tasks.
        dry_run: list what would run without spawning. task: run only this task id.
        max_parallel: cap concurrent running tasks.
        """
        await _ctx_info(
            ctx,
            f"phenotype_thegent_dag_run dry_run={dry_run} task={task} max_parallel={max_parallel} lane={lane}",
        )
        await ctx.report_progress(progress=0, total=2) if ctx is not None else None
        start = time.perf_counter()
        from phenotype_thegent_cli.cli.commands.impl import dag_run_impl
        ...
        await _ctx_info(ctx, f"phenotype_thegent_dag_run spawned={spawned} elapsed={elapsed}ms")
        if ctx is not None:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_dag_sync(cd: str | None = None, auto_run_next: bool = False) -> ToolResult:
        """
        ...
        """
        start = time.perf_counter()
        from phenotype_thegent_cli.cli.commands.impl import dag_sync_impl
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_dag_recover(cd: str | None = None, action: str = "retry-failed") -> ToolResult:
        """
        Perform recovery playbook actions on the DAG.
        action: retry-Union[failed, clear]-Union[stuck, reset]-Union[retries, fallback].
        """
        start = time.perf_counter()
        from phenotype_thegent_cli.cli.commands.impl import dag_recover_impl
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_team_create(
        prompt: str,
        mode: str = "normal",
        teammates: int = 1,
        cd: str | None = None,
        ...
        Returns team_id. Use phenotype_thegent_team_delegate to assign work to teammates.
        """
        start = time.perf_counter()
        root = _get_project_root(Path(cd) if cd else None)
        if not root:
        ...
        config_path.write_text(json.dumps(config, indent=2).decode().decode(), encoding="utf-8")
        elapsed = int((time.perf_counter() - start) * 1000)
        return ToolResult(
            content=json.dumps({"team_id": team_id, "config": config}).decode().decode(),
            structured_content={"team_id": team_id, "config": config},
            meta={"execution_time_ms": elapsed},
        )

    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_team_list(cd: str | None = None) -> ToolResult:
        """
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_team_delegate(
        teammate_id: str,
        prompt: str,
        ...
        try:
            ...
            return ToolResult(
                content=json.dumps({"error": str(e).decode().decode(), "remediation": "Run: thegent teammates list"}),
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_discussion_start(
        topic: str,
        cd: str | None = None,
    ) -> ToolResult:
        """
        Start a discussion/elicitation session. Returns session_id for phenotype_thegent_discussion_add_question.
        """
        start = time.perf_counter()
        root = _get_project_root(Path(cd) if cd else None)
        if not root:
        ...
        elapsed = int((time.perf_counter() - start) * 1000)
        return ToolResult(
            content=json.dumps({"session_id": session_id, "topic": topic}).decode().decode(),
            structured_content={"session_id": session_id, "topic": topic},
            meta={"execution_time_ms": elapsed},
        )

    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_discussion_add_question(
        session_id: str,
        question: str,
        ...
        Use after phenotype_thegent_discussion_start. Call phenotype_thegent_discussion_finalize to save the brief.
        """
        start = time.perf_counter()
        settings = ThegentSettings()
        ...
        if not session_path.exists():
            return ToolResult(
                content=json.dumps({"error": "Session not found", "session_id": session_id}).decode().decode(),
