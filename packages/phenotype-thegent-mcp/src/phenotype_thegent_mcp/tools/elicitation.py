...
def register_elicitation_tools(mcp: FastMCP) -> None:
    ...
    async def phenotype_thegent_elicit_confirmation(
        message: str,
        ctx: Any = None,
    ) -> ToolResult:
        ...
        if ctx is None:
            _log.warning("phenotype_thegent_elicit_confirmation called without ctx; returning unavailable")
    ...
    async def phenotype_thegent_elicit_choice(
        message: str,
        options: list[str],
        ctx: Any = None,
    ) -> ToolResult:
        ...
        if not options:
            ...
            return ToolResult(
                content=json.dumps(error_payload).decode().decode(),
                structured_content=error_payload,
                meta={"execution_time_ms": 0},
            )

        if ctx is None:
            _log.warning("phenotype_thegent_elicit_choice called without ctx; returning unavailable")
            unavail_payload: dict[str, Any] = {"choice": None, "status": "unavailable"}
            return ToolResult(
                content=json.dumps(unavail_payload).decode().decode(),
        ...
        return ToolResult(
            content=json.dumps(payload2).decode().decode(),
    ...
    async def phenotype_thegent_elicit_text(
        message: str,
        placeholder: str = "",
        ctx: Any = None,
    ) -> ToolResult:
        ...
        start = time.perf_counter()
        if ctx is None:
            _log.warning("phenotype_thegent_elicit_text called without ctx; returning unavailable")
            unavail_payload3: dict[str, Any] = {"text": None, "status": "unavailable"}
            return ToolResult(
                content=json.dumps(unavail_payload3).decode().decode(),
        ...
        return ToolResult(
            content=json.dumps(payload3).decode().decode(),
    ...
    async def phenotype_thegent_elicit_structured(
        message: str,
        schema_json: str,
        ctx: Any = None,
    ) -> ToolResult:
        ...
        if ctx is None:
            ...
            return ToolResult(
                content=json.dumps(unavail_payload4).decode().decode(),
        ...
        try:
            ...
            _log.error("phenotype_thegent_elicit_structured: error creating dynamic model: %s", e)
        ...
        return ToolResult(
            content=json.dumps(payload4).decode().decode(),
            structured_content=payload4,
            meta={"execution_time_ms": elapsed},
        )

    mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})(
        phenotype_thegent_elicit_structured
    )

    _log.info(
        "registered elicitation tools: phenotype_thegent_elicit_confirmation, phenotype_thegent_elicit_choice, phenotype_thegent_elicit_text, phenotype_thegent_elicit_structured"
    )
