...
Usage example::
    ...
    from phenotype_thegent_protocols.mcp_tool_patterns import confirm_before_action, ToolAborted

    @confirm_before_action("Delete session {session_id}?")
    async def phenotype_thegent_delete_session(session_id: str, ctx=None):
...
from phenotype_thegent_protocols.mcp.tools.elicitation import elicit_choice, elicit_confirmation
...
def progress_with_fallback(
    ...
    Example::
        ...
        async def phenotype_thegent_bulk_operation(items: list[str], ctx=None, report_step=None):
...
def choice_with_retry(
    ...
    Example::
        ...
        async def phenotype_thegent_set_priority(item_id: str, ctx=None, user_choice=None):
...
def retry_on_error(
    ...
    Example::
        ...
        async def phenotype_thegent_flaky_io(path: str, ctx=None):
            ...
...
def register_tool_pattern_tools(mcp: Any) -> None:
    ...
    Registers:
    - phenotype_thegent_delete_session: uses @confirm_before_action
    - phenotype_thegent_bulk_operation: uses @progress_with_fallback
    ...
    @confirm_before_action("Delete session {session_id}?")
    async def phenotype_thegent_delete_session(
        session_id: str,
        ctx: Any = None,
    ) -> ToolResult:
        ...
        _log.info("phenotype_thegent_delete_session: session_id=%s deleted", session_id)
        return ToolResult(
    ...
    @progress_with_fallback(total_steps=5)
    async def phenotype_thegent_bulk_operation(
        items: list[str],
        ctx: Any = None,
        ...
        _log.info("phenotype_thegent_bulk_operation: processed %d/%d items", len(results), len(items))
        return ToolResult(
            content=json.dumps(payload).decode().decode(),
            structured_content=payload,
            meta={},
    ...
    _log.info(
        ...
        phenotype_thegent_delete_session.__name__,
        phenotype_thegent_bulk_operation.__name__,
    )
