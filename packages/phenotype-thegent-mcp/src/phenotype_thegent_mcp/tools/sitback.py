...
from phenotype_thegent_cli.cli.commands.impl import sitback_dashboard_impl
...
def register_sitback(mcp: "FastMCP") -> None:
    ...
    def resource_sitback_dashboard(profile: str = "medium") -> str:
        ...
        return json.dumps(sitback_dashboard_impl(profile=profile).decode().decode(), sort_keys=True)
    ...
    def phenotype_thegent_sitback_dashboard(profile: str = "medium") -> ToolResult:
        ...
        return ToolResult(
            content=json.dumps(data, sort_keys=True).decode().decode(),
