...
def queue_claim_impl(
    *,
    ...
    if claimed is None:
        return ToolResult(
            content=json.dumps({"claimed": None}).decode().decode(),
            structured_content={"claimed": None},
            meta={"error": "No pending items"},
        )
    return ToolResult(
        content=json.dumps(claimed).decode().decode(),
...
def queue_done_impl(
    *,
    ...
    ok = pq.done(item_id)
    return ToolResult(
        content=json.dumps({"success": ok, "item_id": item_id}).decode().decode(),
...
def queue_add_impl(
    *,
    ...
    return ToolResult(
        content=json.dumps({"success": True, "pending_count": count}).decode().decode(),
