...
def register_research_tools(*, mcp: FastMCP, server_tools_research: Any, logger: logging.Logger) -> tuple[object, ...]:
    ...
    async def thegent_ddg_search(
        ...
        """
        return await server_tools_research.thegent_ddg_search_impl(
            query=query,
    ...
    def thegent_reddit_search(query: str, num_results: int = 5) -> ToolResult:
        ...
        Args:
            ...
            num_results: Max results to return (min: 1, max: 20, default: 5)
        """
        return server_tools_research.thegent_reddit_search_impl(
            query=query,
            num_results=num_results,
        )
    ...
    async def thegent_scrape_url(
        ...
        """
        return await server_tools_research.thegent_scrape_url_impl(
            url=url,
    ...
    def thegent_deep_research(query: str, subreddits: str | None = None) -> ToolResult:
        ...
        """
        return server_tools_research.thegent_deep_research_impl(
            query=query,
    ...
    async def thegent_suggest_prompt(
        ...
        """
        return await server_tools_research.thegent_suggest_prompt_impl(
