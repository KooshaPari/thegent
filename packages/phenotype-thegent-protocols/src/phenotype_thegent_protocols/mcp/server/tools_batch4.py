...
def register_batch4_tools(
    *,
    ...
    def phenotype_thegent_list_droids(
        cd: str | None = None,
        ...
        """
        return server_tools_catalog.phenotype_thegent_list_droids_impl(
            cd=cd,
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_list_skills() -> ToolResult:
        """List discoverable skills from the current discovery backend."""
        return server_tools_skills.phenotype_thegent_list_skills_impl(
            backend=skills_backend,
        )

    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_activate_skill(skill_name: str) -> ToolResult:
        """Load and return a skill payload by name."""
        return server_tools_skills.phenotype_thegent_activate_skill_impl(
    ...
    def phenotype_thegent_terminal_route(prompt: str, cd: str | None = None) -> ToolResult:
        """
        Route a prompt to an active terminal session if matching. Falls back to phenotype_thegent_run if none found.
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    def phenotype_thegent_macos_run_script(
        script: str,
        ...
        from phenotype_thegent_platform.automation.macos_desktop import MacOSDesktopAutomation
    ...
    return (
        ...
        phenotype_thegent_list_skills,
        phenotype_thegent_activate_skill,
