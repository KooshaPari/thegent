...
def phenotype_thegent_inspect_impl(
    *,
...
def phenotype_thegent_session_contracts_impl(
    *,
    owner: str | None,
    all: bool,
    missing_only: bool,
    ...
    elapsed_ms = int((time.perf_counter() - start_time) * 1000)
    return ToolResult(
        content=json.dumps(payload).decode(), structured_content=payload, meta={"execution_time_ms": elapsed_ms}
    )


def phenotype_thegent_session_contract_health_trend_impl(
    *,
    payload_type: str,
    owner: str | None,
    all: bool,
...
def phenotype_thegent_observe_summary_impl(
    *,
    limit: int,
    drift_window: int,
...
def phenotype_thegent_dag_status_impl(
    *,
    cd: str | None,
