...
def handoff_list_impl(
    *,
    ...
    return ToolResult(
        content=json.dumps(snapshots).decode(),
...
def handoff_show_impl(
    *,
    ...
    if not snap:
        return error_result(f"Snapshot {snapshot_id} not found.", "Run phenotype_thegent_handoff_list", extra={})
    return ToolResult(
        content=json.dumps(snap).decode(),
...
def handoff_confirm_impl(
    *,
    ...
    return ToolResult(
        content=json.dumps({"success": ok, "snapshot_id": snapshot_id}).decode(),
...
def terminal_route_impl(
    *,
    ...
    if pane_id:
        ...
        return ToolResult(
            content=json.dumps({"routed": True, "pane_id": pane_id, "success": success}).decode(),
            structured_content={"routed": True, "pane_id": pane_id, "success": success},
            meta={},
        )
    return ToolResult(
        content=json.dumps({"routed": False, "fallback": "Use phenotype_thegent_run or phenotype_thegent_bg"}).decode(),
        structured_content={"routed": False, "fallback": "Use phenotype_thegent_run or phenotype_thegent_bg"},
        meta={},
    )


def phenotype_thegent_terminal_attach_impl(
    *,
    pane_id: str,
