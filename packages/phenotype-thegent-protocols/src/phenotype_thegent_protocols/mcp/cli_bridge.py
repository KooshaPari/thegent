"""cli_bridge: Lazy accessor for phenotype_thegent_cli functions used by the MCP server.
...
Usage inside protocols:
    from phenotype_thegent_protocols.mcp.cli_bridge import cli
    result = cli.run_impl(...)

The ``cli`` proxy object resolves phenotype_thegent_cli.cli.commands.impl and
...
class _CLIBridge:
    """Lazy proxy to phenotype_thegent_cli.cli.commands.impl and related modules.
    ...
    def _get_impl(self) -> Any:
        if self._impl is None:
            self.__class__._impl = importlib.import_module("phenotype_thegent_cli.cli.commands.impl")
        return self._impl

    def __getattr__(self, name: str) -> Any:
        """Resolve any attribute from phenotype_thegent_cli.cli.commands.impl."""
        impl = self._get_impl()
        try:
            return getattr(impl, name)
        except AttributeError as exc:
            raise AttributeError(f"phenotype_thegent_cli.cli.commands.impl has no attribute {name!r}") from exc
    ...
    def auto_init_on_startup(self) -> Any:
        """Lazy accessor for phenotype_thegent_cli.ide.auto_init.auto_init_on_startup."""
        if self._auto_init is None:
            mod = importlib.import_module("phenotype_thegent_cli.ide.auto_init")
    ...
    def ps_impl(self) -> Any:
        ...
        if self._session_ops is None:
            self.__class__._session_ops = importlib.import_module("phenotype_thegent_cli.cli.commands.session_ops_impl")
    ...
    def session_send_impl_raw(self) -> Any:
        ...
        if self._session_ctrl is None:
            self.__class__._session_ctrl = importlib.import_module("phenotype_thegent_cli.cli.commands.session_control_impl")
