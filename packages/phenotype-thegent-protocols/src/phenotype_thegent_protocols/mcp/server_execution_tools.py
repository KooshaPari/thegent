...
def register_execution_tools(
    *,
    ...
    async def phenotype_thegent_config_resolve(
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def phenotype_thegent_negotiate_contract(
        contract_id: str,
    ...
    async def phenotype_thegent_run(
        ...
        Args:
            ...
            async_task: Return task_id immediately and execute in background. Use phenotype_thegent_status to track.
        ...
        if model and not agent:
            from phenotype_thegent_core.models import resolve_route, resolve_route_contract
            ...
            from phenotype_thegent_core.models import ModelCatalog, resolve_route, resolve_route_contract
        ...
        await ctx.info(f"phenotype_thegent_run agent={agent} cd={cd} timeout={timeout}")
        ...
        if async_task:
            from phenotype_thegent_protocols.mcp.task_registry import get_task_registry as _gtr
    ...
    async def phenotype_thegent_loop(
        prompt: str,
        todo_spec: str,
        agent: str | None = None,
        ...
        """
        await ctx.info(f"phenotype_thegent_loop agent={agent} mode={mode}")
        cd_path = Path(cd) if cd else default_cwd
        cwd = resolve_cwd(cd_path)
        if cwd is None:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    async def phenotype_thegent_loop_takeover(
        session_id: str,
        prompt: str,
    ) -> str:
        """
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    async def phenotype_thegent_loop_stop(
        session_id: str,
    ) -> str:
        """
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    async def phenotype_thegent_bg(
        prompt: str | None = None,
        agent: str | None = None,
        cd: str | None = None,
        mode: str = "write",
        ...
        await ctx.info(f"phenotype_thegent_bg agent={agent} cd={cd} owner={owner}")
        cd_path = Path(cd) if cd else default_cwd
        ...
        if span and span.is_recording():
            ...
            if model is not None:
                ...
                span.set_attribute("model", "auto")
            span.set_attribute("provider", provider or requested_provider or "auto")
            span.set_attribute("mode", mode)
            span.set_attribute("timeout_seconds", timeout)
            if owner_tag is not None:
                span.set_attribute("owner", owner_tag)
            else:
                span.set_attribute("owner", "unknown")

        if include_contract and model:
            try:
                from phenotype_thegent_core.models import resolve_route_contract
                from phenotype_thegent_core.models import route_contract as catalog_route_contract
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    async def phenotype_thegent_free(
        prompt: str,
        cd: str | None = None,
        mode: str = "write",
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": False})
    async def phenotype_thegent_flash(
        prompt: str,
        ...
        """
        from phenotype_thegent_agents.agents.flash_agent import flash as _flash
