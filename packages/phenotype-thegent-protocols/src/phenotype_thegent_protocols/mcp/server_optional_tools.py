...
def register_storage_event_tools(
    *,
    ...
    def phenotype_thegent_storage_get(key: str) -> ToolResult:
    ...
    def phenotype_thegent_storage_set(
        key: str,
        value: str,
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": False})
    def phenotype_thegent_events_emit(event_type: str, payload: str) -> ToolResult:
        start_ms = time.monotonic()
        try:
            ...
            return tool_result_type(
                content=json.dumps(err_payload).decode(),
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    def phenotype_thegent_events_replay(since_id: str | None = None) -> ToolResult:
        start_ms = time.monotonic()
    ...
    return {
        "phenotype_thegent_storage_get": phenotype_thegent_storage_get,
        "phenotype_thegent_storage_set": phenotype_thegent_storage_set,
        "phenotype_thegent_events_emit": phenotype_thegent_events_emit,
        "phenotype_thegent_events_replay": phenotype_thegent_events_replay,
    }
...
def register_optional_tools(
    *,
    ...
    try:
        ...
        @mcp.tool()
        async def phenotype_thegent_configure_agent(ctx: Any = current_context()) -> str:
        ...
        @mcp.tool()
        async def phenotype_thegent_approve_deployment(project: str, ctx: Any = current_context()) -> str:
        ...
        exports["phenotype_thegent_configure_agent"] = phenotype_thegent_configure_agent
        exports["phenotype_thegent_approve_deployment"] = phenotype_thegent_approve_deployment
    except Exception as e:
    ...
    try:
        ...
        log.info(
            "storage/event tools registered: phenotype_thegent_storage_get, phenotype_thegent_storage_set, "
            "phenotype_thegent_events_emit, phenotype_thegent_events_replay"
        )
