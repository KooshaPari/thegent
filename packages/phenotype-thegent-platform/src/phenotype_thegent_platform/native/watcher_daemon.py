...
Usage::
    ...
    from phenotype_thegent_platform.native.watcher_daemon import WatchEvent, WatchSpec, get_watcher_daemon
...
def _try_get_breaker() -> Any:
    ...
    try:
        from phenotype_thegent_core.config import ThegentSettings
        from phenotype_thegent_platform.native.state_shm import CircuitBreakerShm

        settings = ThegentSettings()
        shm_default = str(Path(tempfile.gettempdir()) / "phenotype_thegent_watcher.shm")
...
class WatcherDaemon:
    ...
    def _storage_cleanup_loop(self) -> None:
        ...
        from phenotype_thegent_core.config import ThegentSettings
        from phenotype_thegent_execution.orchestration.pruning.prune import prune_stale_shadow_and_logs
