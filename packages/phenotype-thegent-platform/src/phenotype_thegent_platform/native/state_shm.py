...
``phenotype_thegent_shm`` PyO3 Rust extension when available.  When the native extension
...
Usage::
    ...
    from phenotype_thegent_platform.native.state_shm import CircuitBreakerShm, XpTracker
...
def _try_import_native() -> Any | None:
    """Attempt to import the optional phenotype_thegent_shm Rust extension."""
    try:
        import phenotype_thegent_shm

        return phenotype_thegent_shm
    except ImportError:
        return None
...
if _NATIVE_ENABLED and _native_module is None:
    _log.debug(
        "phenotype_thegent_shm native extension not found. Using pure-Python state_shm fallback. "
        "To enable native SHM, build crates/thegent-shm with maturin and install the wheel."
    )


def is_native_available() -> bool:
    """Return True if the Rust phenotype_thegent_shm extension is loaded and usable."""
