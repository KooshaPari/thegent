...
def _thegent_git_has(name: str) -> bool:
    return phenotype_thegent_git is not None and hasattr(phenotype_thegent_git, name)
...
class GitParallelismManager:
    ...
    def _get_ref_hash(self, ref: str) -> str | None:
        ...
        if _thegent_git_has("rev_parse"):
            sha = phenotype_thegent_git.rev_parse(str(self.project_root), ref)
    ...
    def try_auto_merge_commit(
        ...
        *,
        ...
        base = phenotype_thegent_git.merge_base(str(self.project_root), ours_commit, theirs_commit)
        if not base:
            return None

        diff = phenotype_thegent_git.diff_stat(str(self.project_root), f"{ours_commit}..{theirs_commit}")
        if not diff:
            return None

        commit_hash = phenotype_thegent_git.create_commit(
