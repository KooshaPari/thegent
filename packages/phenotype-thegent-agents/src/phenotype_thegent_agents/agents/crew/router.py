...
from phenotype_thegent_core.infra.runtime_dispatcher import router_dispatcher
...
def _register_native_router() -> None:
    ...
    try:
        phenotype_thegent_router = _NATIVE_IMPORTER("phenotype_thegent_router")
        from phenotype_thegent_core.config import get_settings

        settings = get_settings()
        router = phenotype_thegent_router.PyParetoRouter.with_full_config(
...
class RouterManager:
    ...
    def refresh_from_mesh(self, mesh_root: Path | None = None) -> None:
        ...
        from phenotype_thegent_core.infra.ipc import SharedStateManager
