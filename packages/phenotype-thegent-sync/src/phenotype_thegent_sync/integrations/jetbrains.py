...
class JetBrainsIntegration:
    ...
    def write_mcp_config(self, config: JetBrainsConfig) -> Path:
        ...
        phenotype_thegent_entry: dict = {"url": config.mcp_server_url}
        if config.serena_project_root:
            phenotype_thegent_entry["env"] = {"SERENA_PROJECT_ROOT": config.serena_project_root}

        mcp_servers["thegent"] = phenotype_thegent_entry
