...
from phenotype_thegent_core.infra import yaml_load
from phenotype_thegent_platform.platform_paths import get_bin_dir, get_config_dir
from phenotype_thegent_platform.thg_platform import detect_platform
...
class ManageDevkitIntegration:
    ...
    def integrate_paths(self) -> None:
        ...
        if "config_dir" in self.manage_config.get("paths", {}):
            ...
            phenotype_thegent_config_dir = get_config_dir()
            ...
            if manage_config_dir.exists():
                ...
                phenotype_thegent_config_file = shared_config / "config.yaml"
                if not phenotype_thegent_config_file.exists():
                    source_config = phenotype_thegent_config_dir / "config.yaml"
                    if source_config.exists():
                        try:
                            phenotype_thegent_config_file.symlink_to(source_config)
                        except OSError:
                            # Symlink failed, copy instead
                            import shutil

                            shutil.copy2(source_config, phenotype_thegent_config_file)
    ...
    def integrate_tools(self) -> None:
        ...
        phenotype_thegent_bin = get_bin_dir() / "thegent"
        manage_thegent = manage_bin_dir / "thegent"

        if phenotype_thegent_bin.exists() and not manage_thegent.exists():
            try:
                manage_thegent.symlink_to(phenotype_thegent_bin)
            except OSError:
                # Symlink failed, skip
                pass
    ...
    def register_with_manage(self) -> None:
        ...
        if not existing:
            self.manage_config["tools"].append(phenotype_thegent_entry)
