...
def plan_next(
    ...
    from thegent_cli.commands.cli import dag_ready_cmd
...
def plan_status(
    ...
    from thegent_cli.commands.cli import dag_status_cmd
...
def plan_add(
    ...
    from thegent_cli.commands.cli import dag_add_cmd
...
def plan_remove(task_id: str = typer.Argument(..., help="Task ID to remove")):
    from thegent_cli.commands.cli import dag_remove_cmd
...
def plan_roadmap(
    ...
    from thegent_cli.commands.cli_initiative import initiative_list_cmd
...
def plan_work_stream(
    ...
    from thegent_cli.commands.cli import plan_progress_cmd
...
def plan_analyze(
    ...
    from thegent_cli.commands.cli import plan_analyze_cmd
...
def plan_checkpoint(reason: str = typer.Option("Manual checkpoint", "--reason", "-r")):
    from thegent_cli.commands.cli import dag_checkpoint_cmd
...
def plan_rollback(checkpoint_id: str = typer.Argument(..., help="Checkpoint ID to rollback to")):
    from thegent_cli.commands.cli import dag_rollback_cmd
...
def plan_incorporate(dry_run: bool = typer.Option(False, "--dry-run", "-n")):
    from thegent_cli.commands.cli import plan_incorporate_cmd
...
def plan_claim(
    ...
    from thegent_cli.commands.cli import plan_claim_cmd
...
def plan_complete(
    ...
    from thegent_cli.commands.cli import plan_complete_cmd
...
def plan_verify_workstream(
    ...
    from thegent_cli.commands.cli import plan_verify_workstream_cmd
...
def plan_lint_workstream(cd: Path | None = typer.Option(None, "--cd", help="Working directory")) -> None:
    from thegent_cli.commands.cli import plan_lint_workstream_cmd
...
def plan_normalize_workstream(cd: Path | None = typer.Option(None, "--cd", help="Working directory")) -> None:
    from thegent_cli.commands.cli import plan_normalize_workstream_cmd
...
def plan_sessions(
    ...
    from thegent_cli.commands.impl import harness_interact_impl

    if harness:
        result = harness_interact_impl(harness=harness, action="list_sessions")
    else:
        from thegent_cli.commands.impl import harness_list_actions_impl
...
def plan_harness_status(
    ...
    from thegent_cli.commands.impl import harness_list_actions_impl
    from thegent_agents.unified_session_index import HarnessType
