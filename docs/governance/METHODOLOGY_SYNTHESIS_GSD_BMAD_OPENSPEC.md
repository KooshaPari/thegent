# Methodology Synthesis: GSD + BMAD + OpenSpec + thegent

**Status:** Active Reference
**Date:** 2026-02-24
**Purpose:** Extract the best process ideas from GSD, BMAD, and OpenSpec and integrate them
...
| **Verification model** | Nyquist (test-map before code) + goal-backward verifier | Dev story DoD checklist + QA agent | openspec validate --strict before approval |
...
- **Integration:** Add as a required field in the OpenSpec `tasks.md` format: each task must include a `verify:` line with a runnable command.
...
- **Integration:** thegent already uses `docs/`, `openspec/changes/`, `contracts/` for this. Formalize the convention: all cross-agent state lives in files, never in agent memory.
...
- **Integration:** For M/L/XL changes, require `design.md` in the OpenSpec change dir (already part of OpenSpec for multi-system changes). Treat it as the consistency contract for all parallel sub-agents.
...
- **Integration:** The `blocked/` worktree state IS this gate. A worktree cannot move to `active/` until the proposal passes `openspec validate --strict`.

**Keep:** Proactive conflict detection
- Before scaffolding a change, check active changes for overlapping specs.
- **Integration:** Add `openspec list` + `openspec list --specs` to the pre-worktree-creation checklist.
...
| OpenSpec | Sequential-only within a change | thegent prefers wave parallelism where safe; override for M/L scale |
...
### Phase 0 — Context Load (from OpenSpec + BMAD)

Before any worktree is created:

```
1. openspec list                          # What changes are active?
2. openspec list --specs                  # What capabilities exist?
3. git worktree list                      # What worktrees are active?
4. Read openspec/project.md               # Project conventions
...
### Phase 2 — Propose (from OpenSpec + BMAD architecture)

For M/L/XL scale changes:

```
openspec/changes/<change-anchor>/
...
Run `openspec validate <id> --strict`. Resolve every issue. Get approval.
...
4. Run `openspec validate <id> --strict` one final time.
...
2. `openspec archive <change-anchor> --yes`
...
|--------|-----------|---------|
| Proposal ready for review | `openspec validate --strict` passes + PR/comment on proposal.md | Implementation approved to begin |
...
| Story/change complete | All verify commands pass + `openspec validate --strict` passes + quality gate passes | Safe to archive and merge |
...
| Scale | OpenSpec Proposal? | Wave Decomposition? | design.md? | Worktree Mode |
|-------|--------------------|--------------------|-----------|--------------------|
...
9. **Specs = current truth; changes = proposed truth** (OpenSpec) — never mix what IS with what SHOULD BE.
10. **Hard approval gate** (OpenSpec) — worktrees do not leave `blocked/` without validated proposal approval.
11. **Simplicity by default** (OpenSpec) — smallest correct implementation first; escalate with evidence.
...
| OpenSpec | `/Users/kooshapari/CodeProjects/Phenotype/repos/archive/crun/openspec/` (local usage); [github.com/Fission-AI/OpenSpec](https://github.com/Fission-AI/OpenSpec) |
