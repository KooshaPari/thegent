...
**Status:** Reference  
**Date:** 2026-02-15  
**Context:** Model-first routing (task_router → catalog.resolve_route) returns `(provider, model_alias)`. The execution layer uses CLIProxyAPIPlus fork at `../CLIProxyAPIPlus-fork/`. This doc aligns catalog providers with fork capabilities.
...
4. **Fork binary:** Document build step: `cd ../CLIProxyAPIPlus-fork && go build -o cli-proxy-api-plus ./cmd/server`
