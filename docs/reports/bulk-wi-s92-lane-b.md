### [WL-10230]
**Title:** Preserve hook control by separating pre and post execution branches
**Source:** [thegent/src/thegent/mcp/server.go:230]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10231]
**Title:** Preserve sync correctness by separating discovery and conflict resolution
**Source:** [thegent/src/thegent/queue/storage.go:249]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10232]
**Title:** Preserve CLI flow by separating argument parsing from action dispatch
**Source:** [thegent/src/thegent/hooks/dispatcher.go:268]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10233]
**Title:** Preserve queue health by separating intake and processing throttles
**Source:** [thegent/src/thegent/observability/prometheus.go:287]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10234]
**Title:** Preserve observability by separating event ingestion and metric emission
**Source:** [thegent/src/thegent/providers/registry.go:306]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10235]
**Title:** Preserve hook control by separating pre and post execution branches
**Source:** [thegent/src/thegent/integrations/gh_project_sync.py:325]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10236]
**Title:** Preserve sync correctness by separating discovery and conflict resolution
**Source:** [thegent/src/thegent/policy/engine.go:344]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10237]
**Title:** Preserve CLI flow by separating argument parsing from action dispatch
**Source:** [thegent/src/thegent/automation/workflow.go:363]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10238]
**Title:** Preserve queue health by separating intake and processing throttles
**Source:** [thegent/src/thegent/commands/dispatch.go:382]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.

### [WL-10239]
**Title:** Preserve observability by separating event ingestion and metric emission
**Source:** [thegent/src/thegent/runner/runner.go:401]
**Acceptance checklist:
- [ ] Separate parse and execution paths.
- [ ] Preserve current behavior on both happy and failure paths.
- [ ] Add regression tests around boundary conditions.
**Notes:** Continuation artifact for high-volume work item stream.
