# Smart Contract System — thegent, trace, sharecli Roles

**Purpose:** Handoff doc explaining how thegent, trace, and sharecli participate in the smart contract QA governance system.
...
|---------|------------------------------|
| **sharecli** | Hosts the smart contract system: gate script, hooks, governance, P7–P16 phases |
| **trace** | Strictness reference for rollout; requirement traceability system; uses thegent for agent runs |
| **thegent** | Agent orchestration CLI; single source of truth; integrates with sharecli via discovery/dev health |

---

## 2. sharecli — Host of the Smart Contract System
...
- sharecli uses thegent for external agent orchestration
- Example: `thegent bg -d "$(pwd)" --owner "$USER:sharecli" -f cursor '...'`
...
- trace provides the **baseline** for what "strict" means when sharecli rolls out governance to other projects
...
### 4.2 Integration with sharecli

- `thegent discovery register` — registers projects with sharecli
- `sharecli dev health` — delegates to harness; can be used to check health of thegent-managed sessions
- sharecli and trace both invoke thegent directly; no wrappers
...
└───────────────────────────────┬─────────────────────────────────┘
                                ...
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│  sharecli: Conversion pipeline → canonical records → ledger     │
...
│  (trace, sharecli, other projects invoke thegent)               │
└───────────────────────────────┬─────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│  sharecli: qa-smart-contract-gate.py                            │
...
| Doc | Purpose |
|-----|---------|
| `sharecli/docs/guides/QA_GOVERNANCE_SMART_CONTRACT_PLAN_V3.md` | Full v3 plan |
| `sharecli/chatgpt.md` | CDDL origin (chatgpt transcript) |
| `sharecli/docs/unified/modules/governance/smart-contracts.md` | Module overview |
| `sharecli/docs/reports/2026-02-14-SHARECLI-SMART-CONTRACT-HOOKS-AUDIT.md` | Audit and gaps |
| `sharecli/docs/reports/2026-02-14-THGENT-PLAN-AUDIT-FULL.md` | thegent/sharecli/trace CLI audit |
...
2. **sharecli** owns the gate, hooks, and governance; **trace** is the strictness reference; **thegent** runs agents.
