...
> **Source**: `src/thegent/utils/routing_impl/litellm_router.py`
...
```python
build_dynamic_fallback_router(models: list[str], base_policy: str)
```
...
**Returns**: Router configured with the given fallback chain
...
## build_openrouter_model_entry

```python
build_openrouter_model_entry(model_alias: str, openrouter_model_id: str, api_key: Any)
```
...
**Returns**: LiteLLM model_list entry dict for the OpenRouter model
...
**Returns**: Dict mapping provider name to state string: ``"closed"``, ``"open"``,
or ``"half-open"``.
...
**Returns**: Configured LiteLLM Router (possibly shared from cache)
