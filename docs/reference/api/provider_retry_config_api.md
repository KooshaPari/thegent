# provider_retry_config API Reference

> **Source**: `src/thegent/utils/provider_retry_config.py`
...
#### ProviderRetryConfig.retry_after_default

```python
retry_after_default(self: Any)
```
...
```python
get_error_category(status_code: int)
```
...
```python
get_provider_retry_config(provider: str)
```
...
```python
is_retryable_error(status_code: int)
```
