...
```python
routes_for(model_id: str, use_scraped: bool)
```
...
```python
to_catalog_view(use_scraped: bool)
```
...
```python
to_contract_view(use_scraped: bool, provider_filter: Any, use_cache: bool)
```
...
```python
filter_models_for_provider(provider: str, models: list[str])
```
...
```python
normalize_route_policy(policy: Any)
```
...
```python
resolve_route(model_id: str, provider_hint: Any, policy: RoutePolicy, quality_floor: float, lane: Any)
```
...
```python
resolve_route_contract(model_id: str, provider_hint: Any, policy: RoutePolicy)
```
