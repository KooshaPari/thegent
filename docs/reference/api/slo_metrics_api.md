...
```python
__init__(self: Any, output_path: Path)
```
...
```python
emit(self: Any, metric: SloMetric)
```
...
```python
evaluate(self: Any, metric: SloMetric, thresholds: SloThresholds)
```
...
#### SloEmitter.output_path

```python
output_path(self: Any)
```
