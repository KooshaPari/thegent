...
> **Source**: `src/thegent/ui/compositor/compositor.py`
...
```python
__init__(self: Any, ttl: float, error_ttl: float, maxsize: int)
```
...
#### Compositor.add_panel

```python
add_panel(self: Any, panel: Panel)
```
...
```python
cache_stats(self: Any)
```

Return a snapshot of cache performance counters.

**Returns**: A :class:`CacheStats` mapping with keys ``hits``, ``misses``,
...
#### Compositor.get_panel

```python
get_panel(self: Any, name: str)
```
...
**Returns**: Panel instance or None.
...
```python
invalidate(self: Any, panel_name: Any)
```
...
#### Compositor.recover_panel

```python
recover_panel(self: Any, name: str)
```
...
**Returns**: True if panel was found and recovered, False otherwise.

---

#### Compositor.remove_panel

```python
remove_panel(self: Any, name: str)
```
...
**Returns**: ``True`` if the panel existed and was removed, ``False`` otherwise.
...
**Returns**: A list of rendered strings, one per panel in insertion order.
...
**Returns**: Dictionary mapping each panel name to its rendered string.

---

#### Compositor.render_panel

```python
render_panel(self: Any, name: str)
```
...
**Returns**: Rendered string, or *None* if the panel does not exist.
...
#### CompositorProfiler.get_average

```python
get_average(self: Any, panel_id: Any)
```
...
**Returns**: Average ``render_time_ms`` across matching records, or ``0.0``
when there are no matching records.

---

#### CompositorProfiler.get_slowest

```python
get_slowest(self: Any, n: int)
```
...
**Returns**: List of :class:`RenderProfile` instances in descending
``render_time_ms`` order.

---

#### CompositorProfiler.record

```python
record(self: Any, profile: RenderProfile)
```
...
**Returns**: Multi-line formatted string report.
...
**Returns**: Rendered string content, or fallback string on error.
