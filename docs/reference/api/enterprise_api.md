...
> **Source**: `src/thegent/cli/apps/enterprise.py`
...
```python
compliance_audit_export(output: Path, format: str, evidence_path: Path, since_days: Any)
```
...
```python
evidence_list(evidence_path: Path, limit: int)
```
...
```python
evidence_purge(older_than_days: int, evidence_path: Path)
```
...
```python
gdpr_purge(tenant: str, evidence_path: Path, retention_dir: Path)
```
...
```python
keys_rotate(provider: str, new_expires_at: str, webhook_url: str, key_registry: Path)
```
...
```python
keys_status(key_registry: Path, warn_days: int)
```
...
```python
org_create_cmd(name: str, org_registry: Path, initial_tenants: Any)
```
...
```python
org_list_cmd(org_registry: Path)
```
...
```python
org_show_cmd(org_id: str, org_registry: Path)
```
