# agslag API Reference

> **Source**: `src/thegent/tools/agslag.py`

## research_agslag_project

```python
research_agslag_project(workspace_root: Path, query: str, subreddits: Any)
```

Perform comprehensive research on the agslag project.

---

## search_local_agslag

```python
search_local_agslag(workspace_root: Path)
```

Search for agslag related files in the local workspace.

---

## update_research_queue

```python
update_research_queue(results: dict[(str, Any)], queue_file: Path)
```

Update the research queue with new agslag findings.

---

