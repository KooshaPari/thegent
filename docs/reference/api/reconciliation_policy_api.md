# reconciliation_policy API Reference

> **Source**: `src/thegent/integrations/reconciliation_policy.py`
...
#### ReconciliationPolicy.requires_board_id

```python
requires_board_id(self: Any, record: dict)
```
...
**Returns**: True if the record must have a board_id.

---

#### ReconciliationPolicy.validate_record

```python
validate_record(self: Any, record: dict)
```
...
**Returns**: A policy with board_id_first mode and no title fallback.
