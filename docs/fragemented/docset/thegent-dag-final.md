...
```mermaid
flowchart TD
  A0["Intake Request<br/>(WP-1001, P-100)"] --> A1["Classify Scope/Risk/Cost<br/>(WP-1002, FR-EXEC-002)"]
  A1 --> A2{"Schema Valid?<br/>(WP-X1, P-011)"}
  A2 -->|No| A3["Fail Fast + Correction Hints<br/>(WP-X4, FR-ERR-001)"]
  A2 -->|Yes| A4["Hydrate Dependency Graph<br/>(WP-1002, FR-EXEC-003)"]
  A4 --> A5["Compute Priority and Confidence<br/>(WP-1003, FR-EXEC-004)"]
  A5 --> A6{"Policy Pre-check Pass?<br/>(WP-3001, FR-GOV-001)"}
  A6 -->|No| A7["Governance Hold<br/>(WP-3001, P-050)"]
  A6 -->|Yes| A8["Route to Execution Lane<br/>(WP-1001, P-021, P-022)"]
  A7 --> A9["Human Review/Decision<br/>(WP-4001, FR-UX-001)"]
  A9 --> A8
  A8 --> A10["Execute in Bounded Envelope<br/>(WP-1003, FR-EXEC-005, P-060)"]
  A10 --> A11{"Evidence Complete?<br/>(WP-2001, P-100)"}
  A11 -->|No| A12["Repair Evidence + Recheck<br/>(WP-2001, FR-REL-001)"]
  A11 -->|Yes| A13["Integrity and Regression Gate<br/>(WP-2001, FR-GATE-001, P-080)"]
  A12 --> A11
  A13 --> A14{"Gate Pass?<br/>(WP-2001, FR-GATE-002)"}
  A14 -->|No| A15["Checkpoint Rollback<br/>(WP-2001, P-070)"]
  A14 -->|Yes| A16["Promote Phase + Publish Summary<br/>(WP-2001, FR-EXEC-006)"]
  A15 --> A17["Recovery Loop<br/>(R-Root, WP-2001)"]
  A17 --> A8
  A16 --> A18["Close with Audit Artifact<br/>(WP-3001, FR-AUDIT-001)"]
```
...
```mermaid
flowchart TD
  R0["Failure Detected<br/>(WP-2001, P-090)"] --> R1["Classify Failure via MAST 14<br/>(WP-2005, FR-RECOVERY-003)"]
  R1 --> R2{"Known Pattern in Registry?<br/>(WP-2001, P-100)"}
  R2 -->|Yes| R3["Select Recovery Playbook<br/>(WP-2001, FR-RECOVERY-004)"]
  R2 -->|No| R4["Escalate + Provisional Plan<br/>(WP-2001, FR-RECOVERY-005)"]
  R3 --> R5{"Safe To Auto-Run?<br/>(WP-3001, FR-GOV-003)"}
  R5 -->|No| R4
  R5 -->|Yes| R6["Run Recovery with Idempotency<br/>(WP-2004, FR-RECOVERY-006, P-035)"]
  R4 --> R7["Human-Guided Recovery<br/>(WP-4001, FR-UX-002)"]
  R6 --> R8{"Recovered?<br/>(WP-2001, FR-RECOVERY-007)"}
  R7 --> R8
  R8 -->|No| R9["Rollback + Oversight Queue<br/>(WP-2001, FR-RECOVERY-008)"]
  R8 -->|Yes| R10["Post-Recovery Validation<br/>(WP-2001, FR-RECOVERY-009, P-080)"]
  R10 --> R11{"Validation Pass?<br/>(WP-2001, FR-GATE-003)"}
  R11 -->|No| R9
  R11 -->|Yes| R12["Update Learning Registry<br/>(WP-2001, FR-RECOVERY-010, P-100)"]
  R12 --> R13["Close Recovery<br/>(WP-2001, FR-RECOVERY-011)"]
```
...
```mermaid
flowchart TD
  G0["Action Proposed<br/>(WP-3001, FR-GOV-001)"] --> G1["Policy Scope Resolution<br/>(WP-3001, P-050)"]
  G1 --> G2{"Critical Action?<br/>(WP-3001, FR-GOV-002)"}
  G2 -->|Yes| G3["Require Signature + Reason Code<br/>(WP-3001, P-051, P-052)"]
  G2 -->|No| G4["Standard Gate<br/>(WP-3001, P-050)"]
  G3 --> G5{"Signature Valid?<br/>(WP-3001, FR-GOV-004)"}
  G5 -->|No| G6["Block + Governance Queue<br/>(WP-3001, FR-GOV-005)"]
  G5 -->|Yes| G4
  G4 --> G7{"Compliance Constraints Satisfied?<br/>(WP-3001, FR-GOV-006)"}
  G7 -->|No| G6
  G7 -->|Yes| G8["Approve and Emit Audit Event<br/>(WP-3001, FR-AUDIT-001, P-051)"]
  G6 --> G9["Review, Revalidate, or Reject<br/>(WP-4001, FR-UX-003)"]
  G9 --> G4
  G8 --> G10["Record Retention and TTL<br/>(WP-3001, FR-AUDIT-002)"]
```
...
```mermaid
flowchart TD
  S0["Ingress Rate Monitor<br/>(WP-5001, P-090)"] --> S1{"Within Normal Window?<br/>(WP-5001, FR-SCALE-001)"}
  S1 -->|Yes| S2["Normal Scheduling<br/>(WP-1001, P-021)"]
  S1 -->|No| S3["Adaptive Control Mode<br/>(WP-5001, FR-SCALE-002, P-090)"]
  S3 --> S4["Reduce Noncritical Concurrency<br/>(WP-5001, FR-SCALE-003)"]
  S4 --> S5["Protect Critical Lane Capacity<br/>(WP-5001, FR-SCALE-004)"]
  S5 --> S6["Issue Continuity Snapshot<br/>(WP-5001, FR-CONTINUITY-001, P-101)"]
  S6 --> S7{"Stability Restored?<br/>(WP-5001, FR-SCALE-005)"}
  S7 -->|No| S8["Escalate Oversight + Recovery<br/>(WP-4001, FR-UX-004)"]
  S7 -->|Yes| S9["Gradual Return via Hysteresis<br/>(WP-5001, FR-SCALE-006, P-090)"]
  S8 --> S9
  S9 --> S2
```
...
```mermaid
flowchart TD
  C0["All Work Packages Complete<br/>(WP-6001, FR-LAUNCH-001)"] --> C1["Gate A-G Evidence Verified<br/>(WP-2001, FR-GATE-001)"]
  C1 --> C2{"Any Critical Risk Open?<br/>(WP-3001, FR-GOV-007)"}
  C2 -->|Yes| C3["Risk Closure/Acceptance<br/>(WP-3001, FR-GOV-008)"]
  C2 -->|No| C4["Launch Readiness Review<br/>(WP-4001, FR-UX-009)"]
  C3 --> C4
  C4 --> C5{"Two Stable Cycles Passed?<br/>(WP-2001, FR-GATE-004)"}
  C5 -->|No| C6["Remediation + Observe<br/>(WP-2001, FR-RECOVERY-013)"]
  C5 -->|Yes| C7["Program Closure<br/>(WP-6001, FR-LAUNCH-002)"]
  C6 --> C5
```
