    ...
    try:
        with open(file_path) as f:
            ...
            for res in data.get("ddg_results", []):
                links.append({"title": res.get("title", "No Title"), "url": res.get("url", ""), "source": "DuckDuckGo"})
            ...
            for res in data.get("arxiv_results", []):
                links.append({"title": res.get("title", "No Title"), "url": res.get("url", ""), "source": "Arxiv"})

            # Extract from GitHub results
            for res in data.get("github_results", []):
                links.append(
                    {"title": res.get("title", "No Title"), "url": res.get("url", ""), "source": "GitHub (Repo)"}
                )
...
files = Path().glob("drp_results_*.json")
...
queue_path = Path("../docs/research/to-research-queue.md")
