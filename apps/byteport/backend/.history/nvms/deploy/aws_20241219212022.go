...
func createALBListener(AccessKey string, SecretKey string, projectName string, loadBalancerArn string, vpcId string ) (string, error ){
	albClient, err := awsnet.NewALB(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
	...
	targetArn, err := albClient.CreateTargetGroup(context.Background(), projectName+"-"+projectName+"-Byteport", "vpc-0b3b3b3b3b3b3b3b3")
