...
if [[ -f "go.mod" ]]; then
    ...
    # Add Go to path and set required environment variables
    echo 'export PATH=$PATH:/usr/local/go/bin' >> /etc/profile
    echo 'export GOPATH=/root/go' >> /etc/profile
    echo 'export GOMODCACHE=$GOPATH/pkg/mod' >> /etc/profile
    ...
    # Update systemd service ExecStart to use the binary
    ExecStart=/app/$EXTRACT_DIR/%s/app
# Run build commands
