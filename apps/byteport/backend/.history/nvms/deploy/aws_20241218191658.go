...
func provisionNetwork(AccessKey string, SecretKey string, projectName string) (string, error) {
	...
	err = c.CreateHostedZone(context.Background(), projectName+"-Byteport", "us-east-1", "vpc-123456")
	if err != nil {
		fmt.Println(err)
		return "",err
	}
	return projectName+"-Byteport",nil
	
}
func addNewRecord(AccessKey string, SecretKey string, domainName string, recordType string, value string, ttl int) (string, error) {
	c, err := awsnet.NewRoute53(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
	...
	err = c.CreateRecordSet(context.Background(), "zoneID", domainName, recordType, value, ttl)
	if err != nil {
		fmt.Println(err)
		return "",err
	}
