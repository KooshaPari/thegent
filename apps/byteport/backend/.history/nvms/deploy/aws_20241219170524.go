...
func deployEC2(AccessKey string, SecretKey string, bucket S3DeploymentInfo, service models.Service, projectEndpoint string) ([]EC2InstanceInfo,error){
	...
	buildScript := generateBuildScript(bucket, service,AccessKey, SecretKey, projectEndpoint)
...
func generateBuildScript(s3Info S3DeploymentInfo, service models.Service, accessKey, secretKey string, projectEndpoint string) string {
...
Restart=always
Environment=PORT=%d
Environment=BYTE_ENDPOINT=%s

[Install]
WantedBy=multi-user.target
EOF
cat > /etc/profile.d/app-env.sh << EOF
export ENDPOINT_URL="http://your-endpoint-url"
EOF

# Source it for immediate use
...
func provisionNetwork(AccessKey string, SecretKey string, projectName string) (string, string, error) {
	c, err := awsnet.NewRoute53(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
	...
	hostedZoneId, err := c.CreateHostedZone(context.Background(), projectName+"-Byteport", "us-east-1", "vpc-0318e1931eb1c8822")
	if err != nil {
		fmt.Println(err)
		return "","",err
	}
	projectEndpoint := projectName + ".byteport.io"
	return projectEndpoint, hostedZoneId,nil
