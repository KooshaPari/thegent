...
func provisionNetwork(AccessKey string, SecretKey string, projectName string) (string, error) {
	...
	hostedZoneId, err := c.CreateHostedZone(context.Background(), projectName+"-Byteport", "us-east-1", "vpc-123456")
	if err != nil {
		fmt.Println(err)
		return "",err
	}
...
func addNewRecord(AccessKey string, SecretKey string, domainName string, zoneID string, projectName string) (string, error) {
	c, err := awsnet.NewRoute53(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
	...
	err = c.CreateRecordSet(context.Background(), zoneID, domainName, recordType, value, 300, projectName)
	if err != nil {
		fmt.Println(err)
		return "",err
	}
