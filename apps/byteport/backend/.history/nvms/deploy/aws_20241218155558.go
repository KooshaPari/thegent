...
`
	...
	fmt.Println("Script Params: ", accessKey, secretKey, s3Info.BucketName, s3Info.ObjectKey, service.Path, service.Path, buildCommands, service.Name, service.Name, service.Name, service.Path, strings.Join(service.Build, " && "), service.Port, service.Name, service.Name)
...
func pushToS3(zipBall []byte, AccessKey string, SecretKey string, ProjectName string) (S3DeploymentInfo,error) {
	...
	return info,nil*/
	return S3DeploymentInfo{
		BucketName: "test",
		ObjectKey: "src.zip",
		Region: "us-east-1",
		BucketARN: "arn:aws:s3:::test",
		ObjectURL: "https://test.s3.amazonaws.com/src.zip",
		ContentHash: "test",
	},nil
}
